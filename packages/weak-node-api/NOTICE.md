...
- React Native Node API / weak-node-api (callstackincubator/react-native-node-api)
  ...
  - Upstream npm version in this package: tracked via the devDependency "weak-node-api" in package.json
...
- Binary distribution and layout
  - This package does not ship prebuilt binaries. Android/HarmonyOS consumers fetch `libnapi_adapter.so` from remote artifacts when needed; iOS/macOS rely on dynamic symbol lookup.

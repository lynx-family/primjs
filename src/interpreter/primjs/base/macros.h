...
#define vmassert(p, ...)                                                 \
  do {                                                                   \
    if (!(p)) {                                                          \
      base::report_vm_error(__FILE__, __LINE__, "assert(" #p ") failed", \
                            __VA_ARGS__);                                \
    }                                                                    \
  } while (0)
#endif
...
#define unreachable()                                                  \
  do {                                                                 \
    base::report_vm_error(__FILE__, __LINE__, "unreachable here", ""); \
  } while (0)

#define UNIMPLEMENTED()                                                   \
  do {                                                                    \
    base::report_vm_error(__FILE__, __LINE__, "not implemented yet", ""); \
  } while (0)

...
#define DEF_INTERP_HANDLER_RET(name)              \
  ...
  name_r += "_asm_h";                             \
  graph->GetCallDescriptors()->InitCallBcHandler( \
      son::node::CallKind::kFastCall, name_r.c_str(), i);
...
void InterpreterSetup::GenerateBytecodeHandlers(LLVMCodeGen* codegen) {
  ...
  auto bc_size = static_cast<int>(CallBcIndex::kAsmCount1);
  int i = start;
  for (; i < bc_size; i++) {
    auto desc = son::node::CallDescriptors::FastCallBcHandler(i);
    GenerateCode(codegen, generator, desc);
  }
  i++;  // skip kAsmCount1
  bc_size = static_cast<int>(CallBcIndex::kAsmCount2);
  for (; i < bc_size; i++) {

    ...
    if ((opcode == PrimjsOpcode::OP_eq) || (opcode == PrimjsOpcode::OP_neq) ||
      ...
      Bind(&not_equal);
      {
        ...
        if ((opcode == PrimjsOpcode::OP_eq) ||
          ...
          son::node::Label shallow_eq(this);

          auto cond1 = BoolOr(IsLepusString(op1), IsBigIntValue(op1));
          cond1 = BoolOr(cond1, IsLepusRefValue(op1));
          auto cond2 = BoolOr(IsLepusString(op2), IsBigIntValue(op2));
          cond2 = BoolOr(cond2, IsLepusRefValue(op2));
          Branch(BoolAnd(cond1, cond2), &call_slow, &shallow_eq,
                 son::node::BranchHint::kFalse);
          Bind(&shallow_eq);
          {
            ClearNewSp();
...
void HandlerImpl::GenUnaryArithOp(PrimjsOpcode opcode) {
  ...
  Bind(&slow);
  {
    ...
    auto call_opcode = opcode;
    if (opcode == PrimjsOpcode::OP_dec_loc) {
      call_opcode = PrimjsOpcode::OP_dec;
    }

    if (opcode == PrimjsOpcode::OP_inc_loc) {
      call_opcode = PrimjsOpcode::OP_inc;
    }
    auto ret_val = CallRuntimeArg2(desc, ctx, op1, Int8Value((int)call_opcode));
    if ((opcode == PrimjsOpcode::OP_dec_loc) ||
        (opcode == PrimjsOpcode::OP_inc_loc)) {
      idx = Fetch_8(0);
...
void HandlerImpl::GenToPropertyKey(PrimjsOpcode opcode) {
  ...
  {
    ClearNewSp();
    auto desc = son::node::CallDescriptors::LEPUS_ToPropertyKey();
...
son::node::Node* HandlerImpl::FindPropertyForGet(son::node::Node* obj,
  ...
  auto func = [&](son::node::Node* prs, son::node::Node* obj,
    ...
    Bind(&check_succ);
    {
      ...
      auto property_size = Int32Value(sizeof(JSProperty) / sizeof(int64_t));
      // val = pr->u.value;
...
void HandlerImpl::FindPropertyForSet(son::node::Node* obj,
  ...
  auto func = [&](son::node::Node* prs, son::node::Node* h) {
    ...
    Bind(&check_succ);
    {
      ...
      StoreHeapVal(pr, Int32Mul(h_index, property_size), val);
      Jump(&success_exit);
    }
  };
...
void HandlerImpl::GenSetProperty() {
  ...
  Bind(&add_prop);
  {
    ...
    auto val1 = GetTop1();
    StoreHeapVal(pr, Int32Value(0), val1);
    DecSp(2);
    Dispatch(opcode);
  }
...
void HandlerImpl::GenGetGlobalVarCommon(PrimjsOpcode opcode) {
  ...
  auto atom = TruncInt64ToInt32(GetScratch());
  ...
  auto func = [&](son::node::Node* prs, son::node::Node* h) {
    ...
    auto property_size = Int32Value(sizeof(JSProperty) / sizeof(int64_t));
    // val = ppr->u.value;
    auto ret_val = LoadLepusVal(pr, Int32Mul(h_index, property_size));
    Return(ret_val);
  };
...
void HandlerImpl::GenGetGlobalVar(PrimjsOpcode opcode) {
  ...
  if (_use_fast_path) {
    ...
    SetScratch(ZExtToInt64(atom));
    son::node::Node* ret_val = nullptr;
    if (opcode == PrimjsOpcode::OP_get_var) {
      ret_val = CallFastPath(CallBcIndex::kcommon_get_var);
    } else {
      ret_val = CallFastPath(CallBcIndex::kcommon_get_var_undef);
    }
    // ret exception, not found, See common_get_var
    JumpIfException(ret_val, &slow_get);
...
void HandlerImpl::GenGetProperty() {
  son::node::Label slow_get(this);
...
void HandlerImpl::GenGetField(PrimjsOpcode opcode) {
  if (_use_fast_path) {
    ...
    if (!_dispatch_table->is_table2()) {
      SetTop1(obj);
    }
    SetScratch(ZExtToInt64(atom));
    son::node::Label slow_get(this);

    auto ret_val = CallFastPath(CallBcIndex::kcommon_get_property);
    JumpIfException(ret_val, &slow_get);
    {
      ClearNewSp();
...
void HandlerImpl::GenPutField(PrimjsOpcode opcode) {
  if (!_multi_table) {
    auto top0 = LoadSpImpl(-2);
    auto top1 = LoadSpImpl(-1);
    SetTop0Var(top0);
    SetTop1Var(top1);
  }
  DispatchWithId(CallBcIndex::kcommon_set_property);
}
...
void HandlerImpl::GenGetArrayEl(PrimjsOpcode opcode) {
  ...
  auto prop = LoadTop0();
  if (_use_fast_path) {
    auto ret_val = CallFastPath(CallBcIndex::kcommon_get_property_value);
    JumpIfException(ret_val, &slow_get);
    {
...
void HandlerImpl::AddFastArrayElement(son::node::Node* obj,
  ...
  Bind(&set_val);
  {
    ...
    StoreHeapVal(values, len, val);
    StoreArrayCount(obj, new_len);
  }
}

void HandlerImpl::GenPutArrayEl(PrimjsOpcode opcode) {
  if (!_multi_table) {
    auto top0 = LoadSpImpl(-2);
    auto top1 = LoadSpImpl(-1);
    SetTop0Var(top0);
  ...
  SetScratch(this_obj);
  DispatchWithId(CallBcIndex::kcommon_set_property_value);
}
...
void HandlerImpl::GenSetPropertyValue() {
  ...
  Bind(&is_object);
  {
    ...
    if (_use_fast_path) {
      ...
      Bind(&fast_set_int);
      {
        ...
        Bind(&fast_set);
        {
          ClearNewSp();
          auto values = LoadArrayValues(obj_raw);
          StoreHeapVal(values, int_prop, val);
          DecSp(3);
          Dispatch(opcode);
        }
    ...
    Bind(&not_array);
    {
      ...
      auto cond2 =
          LessThanOrEqual(class_id, Int16Value(JS_CLASS_BIG_UINT64_ARRAY));
  ...
  if (_use_fast_path) {
    ...
    {
      ClearNewSp();
      auto desc = son::node::CallDescriptors::LEPUS_add_fast_array_element();
      CallIntRetRuntime(desc, GetCtx(), obj_raw, val);
      DecSp(3);
      Dispatch(opcode);
    }
  }
...
void HandlerImpl::GenArrayFrom(PrimjsOpcode opcode) {
  ...
  auto desc = son::node::CallDescriptors::LEPUS_NewArrayWithArgs();
...
void HandlerImpl::GenSpecialObject(PrimjsOpcode opcode) {
  ...
  Bind(&is_object_var);
  {
    // clean up sp
    ClearNewSp();
    auto desc = son::node::CallDescriptors::LEPUS_NewObjectProto();
...
void HandlerImpl::GenMakeRefOp(PrimjsOpcode opcode) {
  ...
  // pr->u.var_ref = var_ref;
  StoreJsPropertyVarRef(pr, var_ref);
...
#define OP_DEFINE_METHOD_ENUMERABLE 4
  ...
  desc = son::node::CallDescriptors::LEPUS_DefineProperty();
...
void HandlerImpl::GenDoneGenerator(PrimjsOpcode opcode) {
  ...
  auto old_frame = RestoreLastFrame();
  auto pc = GetPc();
  auto offset = AccessBuilder::js_stack_frame_cur_pc_offset(_is_debugger);
  StoreByteOffset(son::node::MachineType::kRawType, old_frame, offset, pc);

  auto val = GetSp();
  offset = AccessBuilder::js_stack_frame_cur_sp_offset(_is_debugger);
  StoreByteOffset(son::node::MachineType::kRawType, old_frame, offset, val);

  auto var_ref_list = RestoreVarRefList();
  auto next = LoadListNext(var_ref_list);
  son::node::Label copy_refs(this);
  son::node::Label done_copy(this);

  auto cond = Equal(next, var_ref_list);
  Branch(cond, &done_copy, &copy_refs);
  Bind(&copy_refs);
  {
    auto desc = son::node::CallDescriptors::prim_copy_var_refs_gc();
    CallRuntimeNoThrow(desc, GetFrame(), old_frame);
    Jump(&done_copy);
  }
  Bind(&done_copy);
...
void HandlerImpl::GenCallOp(PrimjsOpcode opcode) {
  ...
  if (tail_call) {
    ...
    DispatchWithIdArg0(CallBcIndex::kcommon_return, ret_val);
  } else {
    auto frame = GetFrame();
    auto ret_val = RestoreRetVal();
    // pc = sf->cur_pc;
...
void HandlerImpl::GenCallNative(bool from_entry) {
  ...
  auto class_id_offset =
      Int32Mul(ZExtInt16ToInt32(class_id),
               IntValue(sizeof(LEPUSClass) / IntPtrSizeInt()));
  ...
  Branch(cond, &is_null, &not_null, son::node::BranchHint::kFalse);
  Bind(&is_null);
  ...
  SetVar(HandlerVarIndex::kVarBuf, lr);  // var buf is unused
  auto ret_val = Call(desc, call_func, ctx, func_obj, this_obj, call_argc,
                      call_argv, flags);
  auto scratch = GetVar(HandlerVarIndex::kVarBuf);
...
void HandlerImpl::GenCallFromEntry() {
  ...
  Bind(&call_normal);
  {
    ...
    Branch(cond, &arg_mismatch, &arg_match, son::node::BranchHint::kTrue);
    Bind(&arg_match);
    {
...
void HandlerImpl::GenCallGenerator(son::node::Node* func_obj) {
  ...
  auto sf = PushStackFrame(alloc_size, &sf_end);
  CopyArgs(sf, sf_end, old_frame);
  SetVar(HandlerVarIndex::kFrame, sf);
  SaveLastLr();

  SaveLastFrame(old_frame);
  auto rt = LoadRt(ctx);
  ...
  auto func = RestoreCurFunc();
  auto var_refs = LoadVarRefs(CastToRaw(func));
  SaveVarRefsCache(var_refs);
  // cpool = b->cpool;
  auto b = LoadFunctionBytecode(CastToRaw(func));
  ...
  auto var_ref_list_ptr = GetVarRefListAddress(sf);
...
void HandlerImpl::GenCommonCallInternal(son::node::Node* b,
  ...
  auto sf = PushStackFrame(alloca_size);
  // sf->prev_frame = rt->current_stack_frame;
  SavePrevFrame(prev_frame);

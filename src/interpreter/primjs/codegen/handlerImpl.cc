...
void HandlerImpl::GenBinaryArithOp(PrimjsOpcode opcode) {
  ...
  Bind(&both_is_int);
  {
    ...
    if (overflow_check) {
      son::node::Label overflow_to_double(this);
      auto overflow = ExtractValue(res, 1);
      res = ExtractValue(res, 0);
      Branch(overflow, &overflow_to_double, &set_int,
             son::node::BranchHint::kFalse);
      Bind(&set_int);
      ...
      Bind(&overflow_to_double);
      {
        auto left = CastInt32ToDouble(GetLepusInt(op1));
        auto right = CastInt32ToDouble(GetLepusInt(op2));
        son::node::Node* double_res = nullptr;
        if (opcode == PrimjsOpcode::OP_add ||
            opcode == PrimjsOpcode::OP_add_loc) {
          double_res = Float64Add(left, right);
        } else if (opcode == PrimjsOpcode::OP_sub) {
          double_res = Float64Sub(left, right);
        } else {
          vmassert(opcode == PrimjsOpcode::OP_mul, "must be");
          double_res = Float64Mul(left, right);
        }
        if (opcode == PrimjsOpcode::OP_add_loc) {
          StoreLepusVal(var_buf, index, NewFloat64(double_res));
        } else {
          StoreTop0(NewFloat64(double_res));
        }
        Dispatch(opcode);
      }
    } else {
...
void HandlerImpl::GenGetBuf(PrimjsOpcode opcode, int idx, bool is_ref,
  ...
  if (is_ref) {
    ...
    auto pvalue = LoadJSVarRefPValue(var_ref);
...
void HandlerImpl::GenGetGlobalVarCommon(PrimjsOpcode opcode,
  ...
  auto func = [&](son::node::Node* prs, son::node::Node* h) {
    auto flags = LoadJsShapePropertyHashFlags(prs);
    BranchIf(NotEqual(Int32And(flags, IntValue(LEPUS_PROP_TMASK)), IntValue(0)),
...
son::node::Node* HandlerImpl::TryGetInternedStringAtom(
    son::node::Node* prop, son::node::Label* fallback) {
  son::node::Label is_flat_string(this);
  Branch(IsStringValue(prop), &is_flat_string, fallback,
         son::node::BranchHint::kFalse);
  Bind(&is_flat_string);
  auto string = CastToRaw(GetPtr(prop));
  BranchIf(NotEqual(LoadAtomType(string), Int32Value(JS_ATOM_TYPE_STRING)),
           fallback, son::node::BranchHint::kFalse);
  auto desc = son::node::CallDescriptors::prim_get_interned_non_index_atom();
  auto atom = CallRuntimeNoThrow(desc, LoadRt(GetCtx()), string);
  BranchIf(Equal(atom, IntValue(JS_ATOM_NULL)), fallback,
           son::node::BranchHint::kFalse);
  return atom;
}
...
void HandlerImpl::GenGetArrayEl(PrimjsOpcode opcode) {
  ...
  auto prop = LoadTop0();
  if (_use_fast_path) {
    son::node::Label object_ready(this);
    son::node::Label integer_key(this);
    son::node::Label string_key(this);
    son::node::Label fast_string_get(this);
    Branch(IsLepusObject(obj), &object_ready, &slow_get,
           son::node::BranchHint::kTrue);
    Bind(&object_ready);
    Branch(IsLepusInt(prop), &integer_key, &string_key,
           son::node::BranchHint::kTrue);
    Bind(&integer_key);
    ...
    BranchIf(NotEqual(LoadClassId(obj_raw), Int16Value(JS_CLASS_OBJECT)),
             &slow_get, son::node::BranchHint::kFalse);
    BranchIf(LoadObjectIsExotic(obj_raw), &slow_get,
             son::node::BranchHint::kFalse);
    auto atom = TryGetInternedStringAtom(prop, &slow_get);
    Jump(&fast_string_get);
    Bind(&fast_string_get);
    auto ret_val = FindPropertyForGet(obj_raw, atom, &slow_get, nullptr);
    ClearNewSp();
...
void HandlerImpl::GenSetPropertyValue() {
  ...
  Bind(&is_object);
  {
    son::node::Label classify_integer_key(this);
    son::node::Label try_string_key(this);
    son::node::Label is_array(this);
    son::node::Label not_array(this);

    class_id = LoadClassId(obj_raw);
    if (_use_fast_path) {
      Branch(IsLepusInt(prop), &classify_integer_key, &try_string_key,
             son::node::BranchHint::kTrue);
      Bind(&try_string_key);
      {
        son::node::Label add_string_property(this);
        son::node::Label string_property_added(this);
        son::node::Label slow_add_string_property(this);
        son::node::Label string_add_exception(this);
        BranchIf(NotEqual(class_id, Int16Value(JS_CLASS_OBJECT)), &slow_set,
                 son::node::BranchHint::kFalse);
        BranchIf(LoadObjectIsExotic(obj_raw), &slow_set,
                 son::node::BranchHint::kFalse);
        auto atom = TryGetInternedStringAtom(prop, &slow_set);
        FindPropertyForSet(obj_raw, atom, val, &slow_set, &add_string_property);
        ClearNewSp();
        DecSp(3);
        Dispatch(opcode);
        ClearNewSp();

        Bind(&add_string_property);
        BranchIfFalse(LoadObjectIsExtensible(obj_raw), &slow_set,
                      son::node::BranchHint::kFalse);
        FastAddProperty(obj_raw, atom, val, &string_property_added,
                        &slow_add_string_property);

        Bind(&string_property_added);
        ClearNewSp();
        DecSp(3);
        Dispatch(opcode);
        ClearNewSp();

        Bind(&slow_add_string_property);
        ClearNewSp();
        auto desc = son::node::CallDescriptors::add_property_gc();
        auto pr = CallRuntimeNoCheck(desc, GetCtx(), obj_raw, atom,
                                     IntValue(LEPUS_PROP_C_W_E));
        BranchIf(Equal(pr, NullptrValue()), &string_add_exception);
        // add_property_gc may move heap objects. Reload the rooted value from
        // the interpreter stack instead of using the pre-call SSA value.
        auto val_after_gc = LoadTop0();
        StoreHeapVal(GetCtx(), pr, Int32Value(0), val_after_gc);
        DecSp(3);
        Dispatch(opcode);
        ClearNewSp();

        Bind(&string_add_exception);
        DecSp(3);
        DispatchException();
        ClearNewSp();
      }
      Bind(&classify_integer_key);
    }
    auto cond = Equal(class_id, Int16Value(JS_CLASS_ARRAY));
...
void HandlerImpl::GenFclosure(PrimjsOpcode opcode) {
  ...
  auto val = LoadLepusVal(cpool, idx);
  ClearNewSp();
  DispatchWithIdArg0(CallBcIndex::kFastFclosure, val);
}

son::node::Node* HandlerImpl::FastFclosure(son::node::Node* b,
                                           son::node::Label* fallback) {
  ...
  son::node::Variable var_refs(this, son::node::NodeType::RawType(),
                               NullptrValue());
  son::node::Variable closure_vars(this, son::node::NodeType::RawType(),
                                   NullptrValue());
  son::node::Variable parent_var_refs(this, son::node::NodeType::RawType(),
                                      NullptrValue());
  son::node::Variable frame_arg_count(this, son::node::NodeType::IntType(),
                                      IntValue(0));
  ...
  son::node::Variable bound_var_ref(this, son::node::NodeType::RawType(),
                                    NullptrValue());
  Branch(NotEqual(closure_var_count, IntValue(0)), &allocate_var_refs,
  ...
  Jump(&scan_closure_vars);

  BindLoopWithoutJump(&scan_closure_vars, 2);
  auto scan_offset = Int32Mul(*scan_index, IntValue(sizeof(LEPUSClosureVar)));
  auto scan_var = CastToRaw(IntPtrAdd(CastRawToIntPtr(*closure_vars),
                                      ZExtInt32ToIntPtr(scan_offset)));
  ...
  Bind(&scan_local_var);
  {
    ...
    Branch(Equal(*local_var_ref, NullptrValue()), &materialize_local_var_ref,
           &local_var_ref_exists, son::node::BranchHint::kFalse);

    Bind(&materialize_local_var_ref);
    // TryMalloc cannot start concurrent marking, even for existing frame slots.
  ...
  auto var_ref_slot = CastToRaw(IntPtrAdd(
      CastRawToIntPtr(*var_refs), IntPtrMul(ZExtInt32ToIntPtr(*scan_index),
                                            IntPtrValue(sizeof(JSVarRef*)))));
  ...
  Bind(&allocate_object);
  autoinit = LoadByteOffset(son::node::MachineType::kRawType, ctx,
                            AccessBuilder::function_proto_autoinit_offset());
  BranchIf(Equal(*autoinit, NullptrValue()), fallback,
           son::node::BranchHint::kFalse);

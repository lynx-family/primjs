...
#define USE_STACK_POINTER !USE_VIRTUAL_STACK
...
enum class HandlerVarIndex : int32_t {
  ...
  kCpool = (int32_t)HandlerRegister::rCpool,
...
#else
  kFrame = (int32_t)HandlerRegister::rCpool,
#endif
  ...
  kRetVal = kVarBuf,  // use varBuf as RetVal
};
...
class InterpreterAssembler : public CodeAssembler {
 ...
 public:
  ...
  son::node::Node* RestoreThis() {
    auto frame = GetFrame();
    auto offset =
        AccessBuilder::interpreter_frame_frame_this_obj_offset(_is_debugger);
    return LoadByteOffset(son::node::MachineType::kInt64, frame, offset);
  }
  son::node::Node* RestoreNewTarget() {
    auto frame = GetFrame();
    auto offset =
        AccessBuilder::interpreter_frame_frame_new_target_offset(_is_debugger);
    return LoadByteOffset(son::node::MachineType::kInt64, frame, offset);
  }
  ...
  void SavePc(son::node::Node* pc = nullptr) {
    ...
    StoreByteOffset(son::node::MachineType::kRawType, frame, offset,
                    pc ? pc : GetPc());
  }
  ...
  void SaveVarRefsCache(son::node::Node* val) {
    auto frame = GetFrame();
    auto offset = AccessBuilder::interpreter_frame_frame_var_refs_cache_offset(
        _is_debugger);
  ...
  void SaveCpool(son::node::Node* val) {
    auto frame = GetFrame();
    auto offset =
        AccessBuilder::interpreter_frame_frame_cpool_offset(_is_debugger);
#if USE_STACK_POINTER
    SetVar(HandlerVarIndex::kCpool, val);
#endif
    StoreByteOffset(son::node::MachineType::kRawType, frame, offset, val);
  }
  void ReloadCpool() {
...
#if USE_STACK_POINTER
    return GetVar(HandlerVarIndex::kCpool);
#else
    auto frame = GetFrame();
    auto offset =
        AccessBuilder::interpreter_frame_frame_cpool_offset(_is_debugger);
  ...
  son::node::Node* GetVarRefListAddress(son::node::Node* val) {
    auto frame = GetFrame();
    auto offset =
  ...
  void RestoreLastLr() {
    auto frame = GetFrame();
    auto offset =
        AccessBuilder::interpreter_frame_frame_last_lr_offset(_is_debugger);
    auto lr = LoadByteOffset(son::node::MachineType::kRawType, frame, offset);
    SetVar(HandlerVarIndex::kLr, lr);
  }
  ...
  void SaveLastLr() {
    ...
    auto offset =
        AccessBuilder::interpreter_frame_frame_last_lr_offset(_is_debugger);
    StoreByteOffset(son::node::MachineType::kRawType, frame, offset, lr);
  }

  son::node::Node* RestoreLastFrame() {
    auto frame = GetFrame();
    auto offset =
        AccessBuilder::interpreter_frame_frame_last_fp_offset(_is_debugger);
    return LoadByteOffset(son::node::MachineType::kRawType, frame, offset);
  }
  ...
  son::node::Node* RestoreArgc() {
    auto frame = GetFrame();
    auto offset =
        AccessBuilder::interpreter_frame_frame_argc_offset(_is_debugger);
  ...
  son::node::Node* RestoreCallerArgc() {
    auto frame = GetFrame();
    auto offset =
        AccessBuilder::interpreter_frame_frame_caller_argc_offset(_is_debugger);
    return LoadByteOffset(son::node::MachineType::kInt32, frame, offset);
  }

  void SaveCallerArgc(son::node::Node* value) {
    auto frame = GetFrame();
    auto offset =
        AccessBuilder::interpreter_frame_frame_caller_argc_offset(_is_debugger);
    StoreByteOffset(son::node::MachineType::kInt32, frame, offset, value);
  ...
  son::node::Node* RestoreVarRefList() {
    auto frame = GetFrame();
    auto offset =
        AccessBuilder::js_stack_frame_var_ref_list_offset(_is_debugger);
    auto res = IntPtrAdd(frame, IntPtrValue(offset));
    return CastIntPtrToRaw(res);
  }
  ...
  void PopStackFrame(son::node::Node* sf, son::node::Node* prev_sf);
  ...
  void TryDoTransition(DispatchState state_out) {
    _dispatch_table->TryDoTransition(state_out);
  }

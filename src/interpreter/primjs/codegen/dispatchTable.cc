...
void DispatchTable::TransitionToTable0(bool verify_stack) {
  ...
  TryDoTransition(DispatchState::U0_U1);
}
...
void DispatchTable::DoTransition() {
  if (!support_multi_dispatch()) {
    TransitionToTable0();
    return;
  }
  ...
  TryDoTransition(_state_out);
}

void DispatchTable::TryDoTransition(DispatchState state_out) {
  if (is_ext_handler()) {
    return;
  }
  vmassert(state_out != DispatchState::kUnknown, "must be");
  if ((state_out == _current_state) || is_ext_handler()) {
    return;
  }
  auto dispatch_table =
      _assembler->CastRawToIntPtr(_assembler->GetDispatchTable());
  auto offset = _assembler->IntPtrValue((int)PrimjsOpcode::kCount *
                                        _assembler->intptr_size());
  auto diff = (int)state_out - (int)_current_state;
  offset = _assembler->IntPtrMul(offset, _assembler->IntPtrValue(diff));
  auto new_table =
      _assembler->CastToRaw(_assembler->IntPtrAdd(dispatch_table, offset));
  _assembler->SetVar(HandlerVarIndex::kDispatchTable, new_table);
}

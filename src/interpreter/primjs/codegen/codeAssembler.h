...
class CodeAssembler : public son::node::GraphBuilder {
 public:
  ...
  son::node::Node* LoadJSVarRefPValue(son::node::Node* var_ref) {
    auto tagged = LoadByteOffset(son::node::MachineType::kIntptr, var_ref,
                                 AccessBuilder::var_ref_pvalue_off());
    return CastToRaw(IntPtrAnd(
        tagged, IntPtrValue(~static_cast<intptr_t>(JS_VAR_REF_GC_FLAG_MASK))));
  }

  son::node::Node* LoadJSVarRefFlags(son::node::Node* var_ref) {
    auto tagged = LoadByteOffset(son::node::MachineType::kIntptr, var_ref,
                                 AccessBuilder::var_ref_pvalue_off());
    return TruncIntPtrToInt32(
        IntPtrAnd(tagged, IntPtrValue(JS_VAR_REF_GC_FLAG_MASK)));
  }

...
DEFINE_BYTECODE_HANDLER(get_global_ref_undef) {
  son::node::Label slow(this);
  son::node::Label done(this);
  auto func = CastToRaw(GetObject(RestoreCurFunc()));
  auto var_refs = LoadVarRefs(func);
  auto var_ref = LoadRawVal(var_refs, Fetch_16(0));
  auto pvalue = LoadJSVarRefPValue(var_ref);
  son::node::Variable value(this, son::node::NodeType::Int64Type(),
                            LoadLepusVal(pvalue, IntValue(0)));
  Branch(Equal(*value, Uninitialized()), &slow, &done,
         son::node::BranchHint::kFalse);
  Bind(&slow);
  {
    auto desc = son::node::CallDescriptors::JS_GetGlobalRef_GC();
    value =
        CallRuntime(desc, GetCtx(), RestoreCurFunc(), Fetch_16(0), IntValue(0));
    Jump(&done);
  }
  Bind(&done);
  PushSp(*value);
...
DEFINE_BYTECODE_HANDLER(get_global_ref) {
  son::node::Label slow(this);
  son::node::Label done(this);
  auto func = CastToRaw(GetObject(RestoreCurFunc()));
  auto var_refs = LoadVarRefs(func);
  ...
  Bind(&slow);
  {
    auto desc = son::node::CallDescriptors::JS_GetGlobalRef_GC();
    value =
        CallRuntime(desc, GetCtx(), RestoreCurFunc(), Fetch_16(0), IntValue(1));
    Jump(&done);
  }
  Bind(&done);
  PushSp(*value);
...
DEFINE_BYTECODE_HANDLER(put_global_ref) {
  son::node::Label slow(this);
  son::node::Label fast_store(this);
  son::node::Label done(this);
  auto func = CastToRaw(GetObject(RestoreCurFunc()));
  auto var_refs = LoadVarRefs(func);
  auto var_ref = LoadRawVal(var_refs, Fetch_16(0));
  auto pvalue = LoadJSVarRefPValue(var_ref);
  auto value = LoadLepusVal(pvalue, IntValue(0));
  auto flags = LoadJSVarRefFlags(var_ref);
  Branch(BoolOr(Equal(value, Uninitialized()),
                NotEqual(Int32And(flags, IntValue(JS_VAR_REF_GC_CONST)),
                         IntValue(0))),
         &slow, &fast_store, son::node::BranchHint::kFalse);
  Bind(&slow);
  {
    ClearNewSp();
    auto desc = son::node::CallDescriptors::JS_SetGlobalRef_GC();
    CallIntRetRuntime(desc, GetCtx(), RestoreCurFunc(), Fetch_16(0), LoadTop0(),
                      IntValue(0));
    Jump(&done);
  }
  Bind(&fast_store);
  {
    StoreHeapVal(GetCtx(), pvalue, IntValue(0), LoadTop0());
    Jump(&done);
  }
  Bind(&done);
  DecSp();
...
DEFINE_BYTECODE_HANDLER(put_global_ref_init) {
  auto desc = son::node::CallDescriptors::JS_SetGlobalRef_GC();
  ClearNewSp();
  CallIntRetRuntime(desc, GetCtx(), RestoreCurFunc(), Fetch_16(0), LoadTop0(),
                    IntValue(1));
...
DEFINE_BYTECODE_HANDLER(get_ref_value) {
  ...
  if (_use_fast_path) {
    ...
    auto result = FindPropertyForGet(obj_raw, atom, &slow_get, nullptr);
    ClearNewSp();
    PushSp(result);
    Dispatch(opcode);
    Bind(&slow_get);
...
DEFINE_BYTECODE_HANDLER(put_ref_value) {
  ...
  Bind(&not_indefined);
  {
    if (_use_fast_path) {
      ...
      son::node::Label add_property(this);
      son::node::Label property_added(this);
      son::node::Label slow_add_property(this);
      son::node::Label add_exception(this);
      BranchIfFalse(IsLepusObject(*obj_h), &slow_set,
                    son::node::BranchHint::kTrue);
      auto obj_raw = CastToRaw(GetObject(*obj_h));
      BranchIf(NotEqual(LoadClassId(obj_raw), Int16Value(JS_CLASS_OBJECT)),
      ...
      FindPropertyForSet(obj_raw, atom, val, &slow_set, &add_property);
      ClearNewSp();
      DecSp(3);
      Dispatch(opcode);
      ClearNewSp();
      ...
      BranchIfFalse(LoadObjectIsExtensible(obj_raw), &slow_set,
                    son::node::BranchHint::kFalse);
      FastAddProperty(obj_raw, atom, val, &property_added, &slow_add_property);
      ...
      Bind(&slow_add_property);
      ClearNewSp();
      auto add_desc = son::node::CallDescriptors::add_property_gc();
      auto pr = CallRuntimeNoCheck(add_desc, GetCtx(), obj_raw, atom,
                                   IntValue(LEPUS_PROP_C_W_E));
      BranchIf(Equal(pr, NullptrValue()), &add_exception);
...
DEFINE_BYTECODE_HANDLER(spill_table1_to_table0) {
  PushSpImpl(GetTop0());
  DispatchPrevPc();
}

DEFINE_BYTECODE_HANDLER(spill_table2_to_table0) {
  PushSpImpl(GetTop0());
  PushSpImpl(GetTop1());
  DispatchPrevPc();
}
...
DEFINE_BYTECODE_HANDLER(FastFclosure) {
  son::node::Label fallback(this);
  son::node::Label finish(this);
  son::node::Label dispatch_fclosure8(this);
  son::node::Label dispatch_fclosure(this);
  auto ctx = GetCtx();
  auto b = CastToRaw(GetPtr(GetScratch()));
  son::node::Variable result(this, son::node::NodeType::Int64Type(),
                             LepusUndefined());

  result = FastFclosure(b, &fallback);
  Jump(&finish);
  ...
  PushSp(*result);
  auto current_opcode =
      LoadImpl(son::node::MachineType::kInt8, GetPc(), IntValue(-1));
  Branch(Equal(current_opcode,
               Int8Value(static_cast<int>(PrimjsOpcode::OP_fclosure8))),
         &dispatch_fclosure8, &dispatch_fclosure, son::node::BranchHint::kTrue);

  Bind(&dispatch_fclosure8);
  DispatchWithPc(IntPtrAdd(GetPc(), IntPtrValue(1)));

  Bind(&dispatch_fclosure);
  DispatchWithPc(IntPtrAdd(GetPc(), IntPtrValue(4)));
}

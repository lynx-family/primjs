...
DEFINE_BYTECODE_HANDLER(call_constructor) {
  ...
  SavePc(IntPtrAdd(GetPc(), IntValue(2)));
...
DEFINE_BYTECODE_HANDLER(get_super) {
  ...
  // sp[-1] = proto;
  auto ctx = GetCtx();
  auto desc = son::node::CallDescriptors::LEPUS_GetPrototype();
  auto obj = LoadTop0();
...
DEFINE_BYTECODE_HANDLER(put_var) {
  ...
  // }
  auto atom = Fetch_32(0);
  auto desc = son::node::CallDescriptors::LEPUS_SetGlobalVar();
  auto val = LoadTop0();
...
DEFINE_BYTECODE_HANDLER(define_field) {
  ...
  auto desc = son::node::CallDescriptors::LEPUS_DefinePropertyValue();
...
DEFINE_BYTECODE_HANDLER(to_object) {
  ...
  Branch(IsLepusObject(obj), &is_object, &not_object,
         son::node::BranchHint::kTrue);
  Bind(&not_object);
  { DispatchWithIdArg0(CallBcIndex::kSlowToObject, obj); }
  Bind(&is_object);
  Dispatch(opcode);
}
...
DEFINE_BYTECODE_HANDLER(async_iterator_next) {
  ...
  auto this_obj = LoadSp(-4);
  auto desc = son::node::CallDescriptors::LEPUS_Call();
...
DEFINE_BYTECODE_HANDLER(delete_var) {
  ...
  auto atom = Fetch_32(0);
  auto global = GetGlobal();
  auto desc = son::node::CallDescriptors::LEPUS_DeleteProperty();
...
DEFINE_BYTECODE_HANDLER(common_return) {
  ...
  son::node::Label continue_return(this);

  // static inline int list_empty(struct list_head *el) { return el->next == el;
  // }
  auto cond = Equal(next, var_ref_list);
  Branch(cond, &continue_return, &close_refs);
  Bind(&close_refs);
  { DispatchWithId(CallBcIndex::kSlowCloseVarRefs); }
  Bind(&continue_return);
  // TODO lepus_free(ctx, sf->var_refs);
  DispatchFastPath(CallBcIndex::kcommon_return_direct);
}

DEFINE_BYTECODE_HANDLER(common_return_direct) { GenCommonReturn(); }

DEFINE_BYTECODE_HANDLER(common_get_property) { GenGetProperty(); }

DEFINE_BYTECODE_HANDLER(common_set_property) { GenSetProperty(); }

DEFINE_BYTECODE_HANDLER(common_set_property_value) { GenSetPropertyValue(); }

DEFINE_BYTECODE_HANDLER(common_get_property_value) { GenGetPropertyValue(); }

DEFINE_BYTECODE_HANDLER(common_get_var) {
  GenGetGlobalVarCommon(PrimjsOpcode::OP_get_var);
}

DEFINE_BYTECODE_HANDLER(common_get_var_undef) {
  GenGetGlobalVarCommon(PrimjsOpcode::OP_get_var_undef);
}
...
DEFINE_BYTECODE_HANDLER(SlowToObject) {
  auto obj = GetScratch();

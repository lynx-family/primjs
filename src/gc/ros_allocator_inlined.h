...
inline void RosAllocImpl::ReleaseMemorySlotBytes(uint8_t slot,
                                                 size_t internalSize) {
  // Slot validity is checked by the mutator before publishing the object.
  // Workers must not read max_slot_index: the mutator may allocate a new slot.
  size_t ROSIMPL_DUNUSED(previous) =

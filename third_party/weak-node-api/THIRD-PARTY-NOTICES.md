...
This package embeds a subset of Node-Addon-API C++ headers in its `include/` tree for downstream consumption.
...
This package indirectly consumes the Node-API C header definitions (for example `js_native_api.h`, `js_native_api_types.h`, `node_api.h`, `node_api_types.h`) via the upstream `weak-node-api` and `node-api-headers` npm packages. When the `prepare:headers` pipeline is run, these headers may be copied into this package's `include/` directory.

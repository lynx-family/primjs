...
- Node-Addon-API (node-addon-api)
  ...
  - Usage: We merge a subset of Node-Addon-API header files (e.g., `napi.h`, `napi-inl.h`, `napi-inl.deprecated.h`) into this package's `include/` directory for downstream consumption.
...
- Node-API C headers (node-api-headers / Node.js)
  ...
  - Usage: Node-API C header definitions (for example `js_native_api.h`, `js_native_api_types.h`, `node_api.h`, `node_api_types.h`) are provided by Node.js through the `node-api-headers` npm package and consumed transitively via `weak-node-api`. These headers may be copied into this package's `include/` directory as part of the `prepare:headers` pipeline.
...
- GN and CMake integration
  - We provide `BUILD.gn`, `CMakeLists.txt`, and build helper scripts under `tools/` to allow downstream projects to build the published sources directly.
  - We support binary publishing scenarios for macOS (framework) and Windows (dynamic library), including packaging of headers and build metadata for reuse.

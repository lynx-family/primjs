...
static inline const void* copy_iov_to_host(IM3Runtime runtime, void* _mem,
                                           __wasi_iovec_t* host_iov,
                                           __wasi_iovec_t* wasi_iov,
                                           int32_t iovs_len) {
...
m3ApiRawFunction(m3_wasi_generic_fd_prestat_get) {
  ...
  WASI_TRACE("fd:%d | type:%d, name_len:%d", fd, buf->pr_type,
             buf->u.dir.pr_name_len);
...
m3ApiRawFunction(m3_wasi_unstable_fd_seek) {
  ...
  WASI_TRACE("fd:%d, offset:%d, whence:%s | result:%d", fd, offset, whstr,
             *result);
...
m3ApiRawFunction(m3_wasi_generic_fd_renumber) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(__wasi_fd_t, from)
...
m3ApiRawFunction(m3_wasi_generic_fd_tell) {
  ...
  __wasi_errno_t ret = __wasi_fd_tell(fd, result);
...
m3ApiRawFunction(m3_wasi_generic_path_readlink) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(__wasi_fd_t, fd)
      m3ApiGetArgMem(const char*, path) m3ApiGetArg(__wasi_size_t, path_len)
          m3ApiGetArgMem(char*, buf) m3ApiGetArg(__wasi_size_t, buf_len)
  ...
  __wasi_errno_t ret =
      __wasi_path_readlink(fd, path, path_len, buf, buf_len, bufused);
...
m3ApiRawFunction(m3_wasi_generic_path_rename) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(__wasi_fd_t, old_fd) m3ApiGetArgMem(
      const char*, old_path) m3ApiGetArg(__wasi_size_t, old_path_len)
      m3ApiGetArg(__wasi_fd_t, new_fd) m3ApiGetArgMem(const char*, new_path)
...
m3ApiRawFunction(m3_wasi_generic_path_symlink) {
  m3ApiReturnType(uint32_t) m3ApiGetArgMem(const char*, old_path)
      m3ApiGetArg(__wasi_size_t, old_path_len) m3ApiGetArg(__wasi_fd_t, fd)
  ...
  __wasi_errno_t ret =
      __wasi_path_symlink(old_path, old_path_len, fd, new_path, new_path_len);
...
m3ApiRawFunction(m3_wasi_generic_path_open) {
  ...
  __wasi_errno_t ret =
      __wasi_path_open(dirfd, dirflags, path, path_len, oflags, fs_rights_base,
                       fs_rights_inheriting, fs_flags, fd);

  WASI_TRACE(
      "dirfd:%d, dirflags:0x%x, path:%s, oflags:0x%x, fs_flags:0x%x | fd:%d",
      dirfd, dirflags, path, oflags, fs_flags, *fd);
...
m3ApiRawFunction(m3_wasi_unstable_path_filestat_get) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(__wasi_fd_t, fd)
      m3ApiGetArg(__wasi_lookupflags_t, flags) m3ApiGetArgMem(const char*, path)
  ...
  __wasi_errno_t ret =
      __wasi_path_filestat_get(fd, flags, path, path_len, &stat);
...
m3ApiRawFunction(m3_wasi_generic_fd_pread) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(__wasi_fd_t, fd) m3ApiGetArgMem(
      __wasi_iovec_t*, wasi_iovs) m3ApiGetArg(__wasi_size_t, iovs_len)
...
m3ApiRawFunction(m3_wasi_generic_fd_write) {
  ...
  __wasi_errno_t ret =
      __wasi_fd_write(fd, (__wasi_ciovec_t*)iovs, iovs_len, nwritten);
...
m3ApiRawFunction(m3_wasi_generic_fd_pwrite) {
  ...
  __wasi_errno_t ret =
      __wasi_fd_pwrite(fd, (__wasi_ciovec_t*)iovs, iovs_len, offset, nwritten);
...
m3ApiRawFunction(m3_wasi_generic_fd_readdir) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(__wasi_fd_t, fd)
      m3ApiGetArgMem(void*, buf) m3ApiGetArg(__wasi_size_t, buf_len)
...
m3ApiRawFunction(m3_wasi_generic_poll_oneoff) {
  m3ApiReturnType(uint32_t) m3ApiGetArgMem(const __wasi_subscription_t*, in)
      m3ApiGetArgMem(__wasi_event_t*, out) m3ApiGetArg(
          __wasi_size_t, nsubscriptions) m3ApiGetArgMem(__wasi_size_t*, nevents)

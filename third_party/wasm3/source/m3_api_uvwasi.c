...
m3ApiRawFunction(m3_wasi_generic_args_sizes_get) {
  m3ApiReturnType(uint32_t) m3ApiGetArgMem(uvwasi_size_t*, argc)
...
m3ApiRawFunction(m3_wasi_generic_environ_sizes_get) {
  m3ApiReturnType(uint32_t) m3ApiGetArgMem(uvwasi_size_t*, env_count)
...
m3ApiRawFunction(m3_wasi_generic_fd_prestat_dir_name) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd)
      m3ApiGetArgMem(char*, path) m3ApiGetArg(uvwasi_size_t, path_len)
...
m3ApiRawFunction(m3_wasi_generic_fd_prestat_get) {
  ...
  WASI_TRACE("fd:%d | type:%d, name_len:%d", fd, prestat.pr_type,
...
m3ApiRawFunction(m3_wasi_unstable_fd_seek) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd)
      m3ApiGetArg(uvwasi_filedelta_t, offset) m3ApiGetArg(uint32_t, wasi_whence)
  ...
  WASI_TRACE("fd:%d, offset:%ld, whence:%s | result:%ld", fd, offset, whstr,
             pos);
...
m3ApiRawFunction(m3_wasi_generic_fd_renumber) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, from)
...
m3ApiRawFunction(m3_wasi_generic_path_create_directory) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd)
      m3ApiGetArgMem(const char*, path) m3ApiGetArg(uvwasi_size_t, path_len)

          m3ApiCheckMem(path, path_len);

  uvwasi_errno_t ret =
      uvwasi_path_create_directory(&uvwasi, fd, path, path_len);
...
m3ApiRawFunction(m3_wasi_generic_path_readlink) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd)
      m3ApiGetArgMem(const char*, path) m3ApiGetArg(uvwasi_size_t, path_len)
          m3ApiGetArgMem(char*, buf) m3ApiGetArg(uvwasi_size_t, buf_len)
  ...
  uvwasi_errno_t ret = uvwasi_path_readlink(&uvwasi, fd, path, path_len, buf,
                                            buf_len, &uvbufused);
...
m3ApiRawFunction(m3_wasi_generic_path_remove_directory) {
  ...
  uvwasi_errno_t ret =
      uvwasi_path_remove_directory(&uvwasi, fd, path, path_len);
...
m3ApiRawFunction(m3_wasi_generic_path_rename) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, old_fd) m3ApiGetArgMem(
      const char*, old_path) m3ApiGetArg(uvwasi_size_t, old_path_len)
      m3ApiGetArg(uvwasi_fd_t, new_fd) m3ApiGetArgMem(const char*, new_path)
          m3ApiGetArg(uvwasi_size_t, new_path_len)

              m3ApiCheckMem(old_path, old_path_len);
  m3ApiCheckMem(new_path, new_path_len);

  uvwasi_errno_t ret = uvwasi_path_rename(
      &uvwasi, old_fd, old_path, old_path_len, new_fd, new_path, new_path_len);

  WASI_TRACE("old_fd:%d, old_path:%s, new_fd:%d, new_path:%s", old_fd, old_path,
             new_fd, new_path);
...
m3ApiRawFunction(m3_wasi_generic_path_symlink) {
  m3ApiReturnType(uint32_t) m3ApiGetArgMem(const char*, old_path)
      m3ApiGetArg(uvwasi_size_t, old_path_len) m3ApiGetArg(uvwasi_fd_t, fd)
  ...
  uvwasi_errno_t ret = uvwasi_path_symlink(&uvwasi, old_path, old_path_len, fd,
...
m3ApiRawFunction(m3_wasi_generic_path_open) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, dirfd)
  ...
  uvwasi_errno_t ret =
      uvwasi_path_open(&uvwasi, dirfd, dirflags, path, path_len, oflags,
                       fs_rights_base, fs_rights_inheriting, fs_flags, &uvfd);

  WASI_TRACE(
      "dirfd:%d, dirflags:0x%x, path:%s, oflags:0x%x, fs_flags:0x%x | fd:%d",
      dirfd, dirflags, path, oflags, fs_flags, uvfd);
...
m3ApiRawFunction(m3_wasi_unstable_path_filestat_get) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd)
      m3ApiGetArg(uvwasi_lookupflags_t, flags) m3ApiGetArgMem(const char*, path)
          m3ApiGetArg(uint32_t, path_len) m3ApiGetArgMem(uint8_t*, buf)
  ...
  uvwasi_errno_t ret =
      uvwasi_path_filestat_get(&uvwasi, fd, flags, path, path_len, &stat);

  WASI_TRACE("fd:%d, flags:0x%x, path:%s | fs.size:%d", fd, flags, path,
             stat.st_size);
...
m3ApiRawFunction(m3_wasi_generic_fd_pread) {
  ...
  uvwasi_errno_t ret = uvwasi_fd_pread(&uvwasi, fd, (const uvwasi_iovec_t*)iovs,
                                       iovs_len, offset, &num_read);
...
m3ApiRawFunction(m3_wasi_generic_fd_read) {
  ...
  ret = uvwasi_fd_read(&uvwasi, fd, (const uvwasi_iovec_t*)iovs, iovs_len,
                       &num_read);
...
m3ApiRawFunction(m3_wasi_generic_fd_readdir) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_fd_t, fd)
      m3ApiGetArgMem(void*, buf) m3ApiGetArg(uvwasi_size_t, buf_len)
  ...
  uvwasi_size_t uvbufused;
  uvwasi_errno_t ret =
      uvwasi_fd_readdir(&uvwasi, fd, buf, buf_len, cookie, &uvbufused);
...
m3ApiRawFunction(m3_wasi_generic_clock_res_get) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(uvwasi_clockid_t, wasi_clk_id)
...
m3ApiRawFunction(m3_wasi_generic_clock_time_get) {
  ...
  uvwasi_timestamp_t t;
  uvwasi_errno_t ret =
      uvwasi_clock_time_get(&uvwasi, wasi_clk_id, precision, &t);
...
m3ApiRawFunction(m3_wasi_generic_poll_oneoff) {
  m3ApiReturnType(uint32_t) m3ApiGetArgMem(const uvwasi_subscription_t*, in)
      m3ApiGetArgMem(uvwasi_event_t*, out) m3ApiGetArg(
          uvwasi_size_t, nsubscriptions) m3ApiGetArgMem(uvwasi_size_t*, nevents)
  ...
  uvwasi_errno_t ret =
      uvwasi_poll_oneoff(&uvwasi, in, out, nsubscriptions, nevents);
...
#define PREOPENS_COUNT 2
  ...
  init_options.argc = 0;  // runtime->argc is not initialized at this point, so
                          // we implement args_get directly
...
M3Result m3_LinkWASIWithOptions(IM3Module module,
                                uvwasi_options_t init_options) {
  ...
  _(SuppressLookupFailure(
      m3_LinkRawFunction(module, "wasi_unstable", "fd_filestat_get", "i(i*)",
                         &m3_wasi_unstable_fd_filestat_get)));
  _(SuppressLookupFailure(
      m3_LinkRawFunction(module, "wasi_snapshot_preview1", "fd_filestat_get",
                         "i(i*)", &m3_wasi_snapshot_preview1_fd_filestat_get)));
  _(SuppressLookupFailure(
      m3_LinkRawFunction(module, "wasi_unstable", "path_filestat_get",
                         "i(ii*i*)", &m3_wasi_unstable_path_filestat_get)));
  _(SuppressLookupFailure(m3_LinkRawFunction(
      module, "wasi_snapshot_preview1", "path_filestat_get", "i(ii*i*)",
  ...
  for (int i = 0; i < 2; i++) {
    ...
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "fd_pread", "i(i*iI*)", &m3_wasi_generic_fd_pread)));
    _(SuppressLookupFailure(
    ...
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "fd_pwrite", "i(i*iI*)", &m3_wasi_generic_fd_pwrite)));
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "fd_read", "i(i*i*)", &m3_wasi_generic_fd_read)));
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "fd_readdir", "i(i*iI*)", &m3_wasi_generic_fd_readdir)));
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "fd_renumber", "i(ii)", &m3_wasi_generic_fd_renumber)));
    _(SuppressLookupFailure(m3_LinkRawFunction(module, wasi, "fd_sync", "i(i)",
                                               &m3_wasi_generic_fd_sync)));
    _(SuppressLookupFailure(m3_LinkRawFunction(module, wasi, "fd_tell", "i(i*)",
                                               &m3_wasi_generic_fd_tell)));
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "fd_write", "i(i*i*)", &m3_wasi_generic_fd_write)));

    _(SuppressLookupFailure(
        m3_LinkRawFunction(module, wasi, "path_create_directory", "i(i*i)",
                           &m3_wasi_generic_path_create_directory)));
    //_     (SuppressLookupFailure (m3_LinkRawFunction (module, wasi,
    //"path_filestat_set_times",  "i(ii*iIIi)",   ))); _ (SuppressLookupFailure
    //(m3_LinkRawFunction (module, wasi, "path_link", "i(ii*ii*i)",   )));
    _(SuppressLookupFailure(m3_LinkRawFunction(module, wasi, "path_open",
                                               "i(ii*iiIIi*)",
                                               &m3_wasi_generic_path_open)));
    _(SuppressLookupFailure(
        m3_LinkRawFunction(module, wasi, "path_readlink", "i(i*i*i*)",
    ...
    _(SuppressLookupFailure(m3_LinkRawFunction(module, wasi, "path_rename",
                                               "i(i*ii*i)",
                                               &m3_wasi_generic_path_rename)));
    _(SuppressLookupFailure(m3_LinkRawFunction(module, wasi, "path_symlink",
                                               "i(*ii*i)",
                                               &m3_wasi_generic_path_symlink)));
    _(SuppressLookupFailure(
        m3_LinkRawFunction(module, wasi, "path_unlink_file", "i(i*i)",
                           &m3_wasi_generic_path_unlink_file)));

    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "poll_oneoff", "i(**i*)", &m3_wasi_generic_poll_oneoff)));
    _(SuppressLookupFailure(
    ...
    _(SuppressLookupFailure(m3_LinkRawFunction(
        module, wasi, "sched_yield", "i()", &m3_wasi_generic_sched_yield)));

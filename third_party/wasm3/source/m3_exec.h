...
#define pushBacktraceFrame() \
  (PushBacktraceFrame(_mem->moduleList->module->runtime, _pc - 1))
#define fillBacktraceFrame(FUNCTION) \
  (FillBacktraceFunctionInfo(_mem->moduleList->module->runtime, function))
...
#define d_outOfBounds                                                       \
  newTrap(ErrorRuntime(                                                     \
      m3Err_trapOutOfBoundsMemoryAccess, _mem->moduleList->module->runtime, \
      "memory size: %zu; access offset: %zu", _mem->length, operand))

#define d_outOfBoundsMemOp(OFFSET, SIZE)                                 \
  newTrap(ErrorRuntime(m3Err_trapOutOfBoundsMemoryAccess,                \
                       _mem->moduleList->module->runtime,                \
        ...
        d_m3CommutativeCmpOp_f(f32, Equal, ==) d_m3CommutativeCmpOp_f(
            ...
            d_m3CompareOp_f(f32, LessThan, <) d_m3CompareOp_f(
                ...
                d_m3CompareOp_f(f32, LessThanOrEqual, <=) d_m3CompareOp_f(
                    ...
                    d_m3CommutativeOp_i(i32, Add, +) d_m3CommutativeOp_i(
                        i64, Add,
                        +) d_m3CommutativeOp_i(i32,
                                               Multiply,
                                                   *) d_m3CommutativeOp_i(i64,
                                                                          Multiply,
                                                                              *)
...
d_m3Op(CallIndirect) {
  ...
  if (M3_LIKELY(tableIndex < module->tableInfo.curSize)) {
    IM3Function function = module->tableElems[tableIndex];

    if (M3_LIKELY(function)) {
      if (M3_LIKELY(type == function->funcType)) {
        if (M3_UNLIKELY(not function->compiled)) r = CompileFunction(function);
...
d_m3Op(CallRawFunction) {
  ...
  outp += snprintf(outp, oute - outp, "%s!%s(", ctx.function->import.moduleUtf8,
...
d_m3Op(MemGrow) {
  ...
  if (M3_LIKELY(numPagesToGrow)) {
    ...
    M3Result r = ResizeMemory(memory, runtime, requiredPages);
    if (r) _r0 = -1;
...
d_m3Op(Loop) {
  ...
  do {
    ...
    d_m3TracePrint("}");
#endif
    // linear memory pointer needs refreshed here because the block it's looping
    // over can potentially invoke the grow operation.
    _mem = memory->mallocated;
  } while (r == _pc);
...
d_m3Op(BranchTable) {
  ...
  if (branchIndex > numTargets) branchIndex = numTargets;  // the default index
...
d_m3Op(ContinueLoop) {
  ...
  // TODO: this is where execution can "escape" the M3 code and callback to the
  // client / fiber switch OR it can go in the Loop operation. I think it's best
  // to do here. adding code to the loop operation has the potential to increase
  // its native-stack usage. (don't forget ContinueLoopIf too.)

...
static M3_NOINLINE M3Result EnsureCodePageNumLines(IM3Compilation o,
                                                   u32 i_numLines) {
  ...
  if (NumFreeLines(o->page) < i_numLines) {
    ...
    if (page) {
      m3log(emit, "bridging new code page from: %d %p (free slots: %d) to: %d",
            o->page->info.sequence, GetPC(o), NumFreeLines(o->page),
            page->info.sequence);
...
static M3_NOINLINE M3Result EmitOp(IM3Compilation o, IM3Operation i_operation) {
  ...
  // it's OK for page to be null; when compile-walking the bytecode without
  // emitting
  if (o->page) {
...
static M3_NOINLINE void EmitConstant32(IM3Compilation o,
                                       const u32 i_immediate) {
...
static M3_NOINLINE pc_t EmitPointer(IM3Compilation o,
                                    const void *const i_pointer) {
...
// just want less letters and numbers to stare at down the way in the compiler
// table
...
static const IM3Operation c_setGlobalOps[] = {
    NULL, op_SetGlobal_i32, op_SetGlobal_i64, FPOP(op_SetGlobal_f32),
    FPOP(op_SetGlobal_f64)};
static const IM3Operation c_setRegisterOps[] = {
    NULL, op_SetRegister_i32, op_SetRegister_i64, FPOP(op_SetRegister_f32),
    FPOP(op_SetRegister_f64)};

static const IM3Operation c_intSelectOps[2][4] = {
    {op_Select_i32_rss, op_Select_i32_srs, op_Select_i32_ssr,
     op_Select_i32_sss},
    {op_Select_i64_rss, op_Select_i64_srs, op_Select_i64_ssr,
     op_Select_i64_sss}};
#if d_m3HasFloat
static const IM3Operation c_fpSelectOps[2][2][3] = {
    {{op_Select_f32_sss, op_Select_f32_srs,
      op_Select_f32_ssr},  // selector in slot
     {op_Select_f32_rss, op_Select_f32_rrs,
      op_Select_f32_rsr}},  // selector in reg
    {{op_Select_f64_sss, op_Select_f64_srs,
      op_Select_f64_ssr},  // selector in slot
     {op_Select_f64_rss, op_Select_f64_rrs,
      op_Select_f64_rsr}}};  // selector in reg
#endif

// all args & returns are 64-bit aligned, so use 2 slots for a
// d_m3Use32BitSlots=1 build
static const u16 c_ioSlotCount = sizeof(u64) / sizeof(m3slot_t);

static M3Result AcquireCompilationCodePage(IM3Compilation o,
                                           IM3CodePage *o_codePage) {
  M3Result result = m3Err_none;
  ...
  if (page) {
#if (d_m3EnableCodePageRefCounting)
    {
      if (o->function) {
        ...
        _(m3ReallocArray(&func->codePageRefs, IM3CodePage,
                         func->numCodePageRefs, index));
...
static inline i16 GetStackTopIndex(IM3Compilation o) {
  d_m3Assert(o->stackIndex > o->stackFirstDynamicIndex or
             IsStackPolymorphic(o));
  return o->stackIndex - 1;
}

// Items in the static portion of the stack (args/locals) are hidden from
// GetStackTypeFromTop () In other words, only "real" Wasm stack items can be
// inspected.  This is important when returning values, etc. and you need an
// accurate wasm-view of the stack.
...
static inline u16 GetExtraSlotForStackIndex(IM3Compilation o,
                                            u16 i_stackIndex) {
  u16 baseSlot = GetSlotForStackIndex(o, i_stackIndex);

  if (baseSlot != c_slotUnused) {
    u16 extraSlot =
        GetTypeNumSlots(GetStackTypeFromBottom(o, i_stackIndex)) - 1;
    baseSlot += extraSlot;
  }
...
static inline void MarkSlotAllocated(IM3Compilation o, u16 i_slot) {
  ...
  o->slotMaxAllocatedIndexPlusOne =
      M3_MAX(o->slotMaxAllocatedIndexPlusOne, i_slot + 1);

  TouchSlot(o, i_slot);
}

static inline void MarkSlotsAllocated(IM3Compilation o, u16 i_slot,
                                      u16 i_numSlots) {
  while (i_numSlots--) MarkSlotAllocated(o, i_slot++);
}

static inline void MarkSlotsAllocatedByType(IM3Compilation o, u16 i_slot,
                                            u8 i_type) {
  u16 numSlots = GetTypeNumSlots(i_type);
  MarkSlotsAllocated(o, i_slot, numSlots);
}

static M3Result AllocateSlotsWithinRange(IM3Compilation o, u16 *o_slot,
                                         u8 i_type, u16 i_startSlot,
                                         u16 i_endSlot) {
...
static inline M3Result AllocateSlots(IM3Compilation o, u16 *o_slot, u8 i_type) {
  return AllocateSlotsWithinRange(o, o_slot, i_type, o->slotFirstDynamicIndex,
                                  d_m3MaxFunctionSlots);
}

static inline M3Result AllocateConstantSlots(IM3Compilation o, u16 *o_slot,
                                             u8 i_type) {
  u16 maxTableIndex = o->slotFirstConstIndex + d_m3MaxConstantTableSize;
  return AllocateSlotsWithinRange(
      o, o_slot, i_type, o->slotFirstConstIndex,
      M3_MIN(o->slotFirstDynamicIndex, maxTableIndex));
}

// TOQUE: this usage count system could be eliminated. real world code doesn't
// frequently trigger it.  just copy to multiple unique slots.
static inline M3Result IncrementSlotUsageCount(IM3Compilation o, u16 i_slot) {
  ...
  // OPTZ (memory): 'm3Slots' could still be fused with 'typeStack' if 4 bits
  // were used to indicate: [0,1,2,many]. The many-case would scan 'wasmStack'
  // to determine the actual usage count
...
static inline void AllocateRegister(IM3Compilation o, u32 i_register,
                                    u16 i_stackIndex) {
...
static M3Result PreserveRegisterIfOccupied(IM3Compilation o,
                                           u8 i_registerType) {
  M3Result result = m3Err_none;
...
// otherwise they'd end up preserve-copied in the block to probably different
// locations (if/else)
...
static M3Result _PushAllocatedSlotAndEmit(IM3Compilation o, u8 i_type,
                                          bool i_doEmit) {
  M3Result result = m3Err_none;
...
static M3Result PushConst(IM3Compilation o, u64 i_word, u8 i_type) {
  ...
  if (numRequiredSlots == 2 and numUsedConstSlots >= 2) {
    ...
    for (u16 slot = firstConstSlot; slot < o->slotMaxConstIndex - 1;
         slot += 2) {
  ...
  if (not matchFound) {
    ...
    if (result ||
        slot ==
            c_slotUnused)  // no more constant table space; use inline constants
    {
      result = m3Err_none;
      ...
      o->slotMaxConstIndex =
          M3_MAX(slot + numRequiredSlots, o->slotMaxConstIndex);
    }
  }
...
static M3Result CopyStackIndexToSlot(IM3Compilation o, u16 i_destSlot,
                                     u16 i_stackIndex)  // NoPushPop
{
  M3Result result = m3Err_none;
...
static M3Result CopyStackTopToSlot(IM3Compilation o,
                                   u16 i_destSlot)  // NoPushPop
{
  M3Result result;
...
// a copy-on-write strategy is used with locals. when a get local occurs, it's
// not copied anywhere. the stack entry just has a index pointer to that local
// memory slot. then, when a previously referenced local is set, the current
// value needs to be preserved for those references

// TODO: consider getting rid of these specialized operations: PreserveSetSlot &
// PreserveCopySlot. They likely just take up space (which seems to reduce
// performance) without improving performance.
static M3Result PreservedCopyTopSlot(IM3Compilation o, u16 i_destSlot,
                                     u16 i_preserveSlot) {
  M3Result result = m3Err_none;
...
// if local is unreferenced, o_preservedSlotNumber will be equal to localIndex
// on return
static M3Result FindReferencedLocalWithinCurrentBlock(
    IM3Compilation o, u16 *o_preservedSlotNumber, u32 i_localSlot) {
  M3Result result = m3Err_none;
...
static M3Result GetBlockScope(IM3Compilation o, IM3CompilationScope *o_scope,
                              u32 i_depth) {
  M3Result result = m3Err_none;
...
static M3Result CopyStackSlotsR(IM3Compilation o, u16 i_targetSlotStackIndex,
                                u16 i_stackIndex, u16 i_endStackIndex,
                                u16 i_tempSlot) {
  M3Result result = m3Err_none;
  ...
  if (i_stackIndex < i_endStackIndex) {
    ...
    if (targetSlot != srcSlot) {
      ...
      while (checkIndex < i_endStackIndex) {
        ...
        if (targetSlot == otherSlot1 or targetSlot == otherSlot2 or
            targetSlot + extraSlot == otherSlot1) {
          _throwif(m3Err_functionStackOverflow,
                   i_tempSlot >= d_m3MaxFunctionSlots);
    ...
    _(CopyStackSlotsR(o, i_targetSlotStackIndex + 1, i_stackIndex + 1,
                      i_endStackIndex, i_tempSlot));
...
static M3Result ResolveBlockResults(IM3Compilation o,
                                    IM3CompilationScope i_targetBlock,
                                    bool i_isBranch) {
  M3Result result = m3Err_none;
  if (d_m3LogWasmStack) dump_type_stack(o);
  ...
  _throwif(m3Err_typeCountMismatch,
           i_isBranch ? (blockHeight < numValues) : (blockHeight != numValues));
  ...
  if (numValues) {
    ...
    u16 tempSlot = o->maxStackSlots;  // GetMaxUsedSlotPlusOne (o); doesn't work
                                      // cause can collide with slotRecords
    AlignSlotToType(&tempSlot, c_m3Type_i64);

    _(CopyStackSlotsR(o, slotRecords, endIndex - numValues, endIndex,
                      tempSlot));
...
static M3Result ReturnValues(IM3Compilation o,
                             IM3CompilationScope i_functionBlock,
                             bool i_isBranch) {
  M3Result result = m3Err_none;
  if (d_m3LogWasmStack) dump_type_stack(o);

  u16 numReturns = GetFuncTypeNumResults(
      i_functionBlock->type);  // could just o->function too...
  u16 blockHeight = GetNumBlockValuesOnStack(o);

  if (not IsStackPolymorphic(o))
    _throwif(m3Err_typeCountMismatch, i_isBranch ? (blockHeight < numReturns)
                                                 : (blockHeight != numReturns));
  ...
  if (numReturns) {
    ...
    for (u16 i = 0; i < numReturns; ++i) {
      u8 returnType =
          GetFuncTypeResultType(i_functionBlock->type, numReturns - 1 - i);

      u8 stackType = GetStackTypeFromTop(
          o, i);  // using FromTop so that only dynamic items are checked
...
static M3Result Compile_Const_i32(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  m3log(compile, d_indent " (const i32 = %" PRIi32 ")", get_indention_string(o),
        value);
_catch:
  return result;
}
...
static M3Result Compile_Const_f32(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  m3log(compile, d_indent " (const f32 = %" PRIf32 ")", get_indention_string(o),
        value.f);
...
static M3Result Compile_Const_f64(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  m3log(compile, d_indent " (const f64 = %" PRIf64 ")", get_indention_string(o),
        value.f);
...
static M3Result Compile_ExtendedOpcode(IM3Compilation o, m3opcode_t i_opcode) {
  _try {
    ...
    m3log(compile, d_indent " (FC: %" PRIi32 ")", get_indention_string(o),
          opcode);
...
static M3Result Compile_SetLocal(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  u32 localIndex;
  _(ReadLEB_u32(&localIndex, &o->wasm,
                o->wasmEnd));  //  printf ("--- set local: %d \n", localSlot);
...
static M3Result Compile_GetGlobal(IM3Compilation o, M3Global *i_global) {
  ...
  IM3Operation op =
      Is64BitType(i_global->type) ? op_GetGlobal_s64 : op_GetGlobal_s32;
  _(EmitOp(o, op));
...
static M3Result Compile_GetSetGlobal(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  if (globalIndex < o->module->numGlobals) {
    if (o->module->globals) {
      ...
      _((i_opcode == c_waOp_getGlobal) ? Compile_GetGlobal(o, global)
                                       : Compile_SetGlobal(o, global));
    } else
      _throw(ErrorCompile(m3Err_globalMemoryNotAllocated, o,
...
static M3Result Compile_Table_Size(IM3Compilation o, m3opcode_t i_opcode) {
  _try {
    u32 tableIndex;
    _(ReadLEB_u32(&tableIndex, &o->wasm, o->wasmEnd));

    _throwif("only one table is supported yet", tableIndex > 0);

    _(PreserveRegisterIfOccupied(o, c_m3Type_i32));

    _(EmitOp(o, op_TblSize));
...
static void EmitPatchingBranchPointer(IM3Compilation o,
                                      IM3CompilationScope i_scope) {
  pc_t patch = EmitPointer(o, i_scope->patches);
  m3log(compile, "branch patch required at: %p", patch);
  i_scope->patches = patch;
}

static M3Result EmitPatchingBranch(IM3Compilation o,
                                   IM3CompilationScope i_scope) {
  M3Result result = m3Err_none;
  ...
  if (scope->opcode == c_waOp_loop) {
    if (i_opcode == c_waOp_branchIf) {
      if (GetFuncTypeNumParams(scope->type)) {
        IM3Operation op = IsStackTopInRegister(o) ? op_BranchIfPrologue_r
  ...
  } else  // forward branch
  {
    ...
    if (i_opcode == c_waOp_branchIf) {
      if (targetHasResults or isReturn) {
        ...
        IM3Operation op =
            IsStackTopInRegister(o) ? op_BranchIf_r : op_BranchIf_s;
...
static M3Result Compile_BranchTable(IM3Compilation o, m3opcode_t i_opcode) {
  _try {
    ...
    _(PreserveRegisterIfOccupied(
        o, c_m3Type_i64));  // move branch operand to a slot
    u16 slot = GetStackTopSlotNumber(o);
    _(Pop(o));

    // OPTZ: according to spec: "forward branches that target a control
    // instruction with a non-empty result type consume matching operands first
    // and push them back on the operand stack after unwinding" So, this
    // move-to-reg is only necessary if the target scopes have a type.

    u32 numCodeLines =
        targetCount +
        4;  // 3 => IM3Operation + slot + target_count + default_target
    ...
    for (u32 i = 0; i < targetCount; ++i) {
      ...
      ReleaseCompilationCodePage(
          o);  // FIX: continueOpPage can get lost if thrown
      o->page = savedPage;
...
static M3Result CompileCallArgsAndReturn(IM3Compilation o, u16 *o_stackOffset,
                                         IM3FuncType i_type,
                                         bool i_isIndirect) {
  _try {
  ...
  _try {
    ...
    if (function) {
      m3log(compile, d_indent " (func= [%d] '%s'; args= %d)",
            get_indention_string(o), functionIndex,
      ...
      if (function->module) {
        ...
        _throw(ErrorCompile(m3Err_functionImportMissing, o, "'%s.%s'",
...
static M3Result Compile_CallIndirect(IM3Compilation o, m3opcode_t i_opcode) {
  _try {
    ...
    _throwif("function call type index out of range",
             typeIndex >= o->module->numFuncTypes);
...
static M3Result ReadBlockType(IM3Compilation o, IM3FuncType *o_blockType) {
  ...
  if (type < 0) {
    ...
    m3log(compile, d_indent " (type: %s)", get_indention_string(o),
          c_waTypes[valueType]);
    ...
    *o_blockType = o->module->funcTypes[type];
    m3log(compile, d_indent " (type: %s)", get_indention_string(o),
          SPrintFuncTypeSignature(*o_blockType));
  }
_catch:
  return result;
}
  ...
  if (o->stackIndex > o->stackFirstDynamicIndex) {
    ...
    for (u32 i = 0; i < numArgsAndLocals; ++i) {
      ...
      if (preservedSlotNumber != slot) {
        u8 type = GetStackTypeFromBottom(o, i);
        d_m3Assert(type != c_m3Type_none) IM3Operation op =
            Is64BitType(type) ? op_CopySlot_64 : op_CopySlot_32;
...
static M3Result Compile_LoopOrBlock(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  if (i_opcode == c_waOp_loop) {
    ...
    if (numParams) {
      ...
      u16 numValues = GetNumBlockValuesOnStack(
          o);  // CompileBlock enforces this at comptime
...
static M3Result CompileElseBlock(IM3Compilation o, pc_t *o_startPC,
                                 IM3FuncType i_blockType) {
  _try {
...
static M3Result Compile_Select(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  if (IsFpType(type)) {
    ...
    // 'sss' operation doesn't consume a register, so might have to protected
    // its contents
    if (not IsStackTopInRegister(o) and not IsStackTopMinus1InRegister(o) and
        not IsStackTopMinus2InRegister(o)) {
      _(PreserveRegisterIfOccupied(o, type));
    }
  ...
  if (opInfo->stackOffset == 0) {
    ...
    if (IsStackTopInRegister(o)) {
      ...
      if (IsStackTopMinus1InRegister(o)) {
        d_m3Assert(i_opcode == c_waOp_store_f32 or
                   i_opcode == c_waOp_store_f64);
...
#ifdef DEBUG
    result =
        ErrorCompile("no operation found for opcode", o, "'%s'", opInfo->name);
#else
...
static M3Result Compile_Load_Store(IM3Compilation o, m3opcode_t i_opcode) {
  _try {
    ...
    m3log(compile, d_indent " (offset = %d)", get_indention_string(o),
          memoryOffset);
    IM3OpInfo opInfo = GetOpInfo(i_opcode);
    _throwif(m3Err_unknownOpcode, not opInfo);
...
// d_logOp, d_logOp2 macros aren't actually used by the compiler, just codepage
// decoding (d_m3LogCodePages = 1)
#define d_logOp(OP) \
...
#define d_binOpList(TYPE, NAME)                                                \
  {                                                                            \
    op_##TYPE##_##NAME##_rs, op_##TYPE##_##NAME##_sr, op_##TYPE##_##NAME##_ss, \
        NULL                                                                   \
  }
...
const M3OpInfo c_operations[] = {
    M3OP("unreachable", 0, none, d_logOp(Unreachable),
         Compile_Unreachable),                                   // 0x00
    ...
    M3OP("br_if", -1, none, d_logOp2(BranchIf_r, BranchIf_s),
         Compile_Branch),  // 0x0d
    M3OP("br_table", -1, none, d_logOp(BranchTable),
         Compile_BranchTable),                                // 0x0e
    M3OP("return", 0, any, d_logOp(Return), Compile_Return),  // 0x0f
    M3OP("call", 0, any, d_logOp(Call), Compile_Call),        // 0x10
    M3OP("call_indirect", 0, any, d_logOp(CallIndirect),
         Compile_CallIndirect),  // 0x11
    M3OP("return_call", 0, any, d_emptyOpList,
         Compile_Call),  // 0x12 TODO: Optimize
    M3OP("return_call_indirect", 0, any, d_emptyOpList,
         Compile_CallIndirect),  // 0x13
    ...
    M3OP("i32.load", 0, i_32, d_unaryOpList(i32, Load_i32),
         Compile_Load_Store),  // 0x28
    M3OP("i64.load", 0, i_64, d_unaryOpList(i64, Load_i64),
         Compile_Load_Store),  // 0x29
    M3OP_F("f32.load", 0, f_32, d_unaryOpList(f32, Load_f32),
           Compile_Load_Store),  // 0x2a
    M3OP_F("f64.load", 0, f_64, d_unaryOpList(f64, Load_f64),
           Compile_Load_Store),  // 0x2b

    M3OP("i32.load8_s", 0, i_32, d_unaryOpList(i32, Load_i8),
         Compile_Load_Store),  // 0x2c
    M3OP("i32.load8_u", 0, i_32, d_unaryOpList(i32, Load_u8),
         Compile_Load_Store),  // 0x2d
    M3OP("i32.load16_s", 0, i_32, d_unaryOpList(i32, Load_i16),
         Compile_Load_Store),  // 0x2e
    M3OP("i32.load16_u", 0, i_32, d_unaryOpList(i32, Load_u16),
         Compile_Load_Store),  // 0x2f

    M3OP("i64.load8_s", 0, i_64, d_unaryOpList(i64, Load_i8),
         Compile_Load_Store),  // 0x30
    M3OP("i64.load8_u", 0, i_64, d_unaryOpList(i64, Load_u8),
         Compile_Load_Store),  // 0x31
    M3OP("i64.load16_s", 0, i_64, d_unaryOpList(i64, Load_i16),
         Compile_Load_Store),  // 0x32
    M3OP("i64.load16_u", 0, i_64, d_unaryOpList(i64, Load_u16),
         Compile_Load_Store),  // 0x33
    M3OP("i64.load32_s", 0, i_64, d_unaryOpList(i64, Load_i32),
         Compile_Load_Store),  // 0x34
    M3OP("i64.load32_u", 0, i_64, d_unaryOpList(i64, Load_u32),
         Compile_Load_Store),  // 0x35

    M3OP("i32.store", -2, none, d_binOpList(i32, Store_i32),
         Compile_Load_Store),  // 0x36
    M3OP("i64.store", -2, none, d_binOpList(i64, Store_i64),
         Compile_Load_Store),  // 0x37
    M3OP_F("f32.store", -2, none, d_storeFpOpList(f32, Store_f32),
           Compile_Load_Store),  // 0x38
    M3OP_F("f64.store", -2, none, d_storeFpOpList(f64, Store_f64),
           Compile_Load_Store),  // 0x39

    M3OP("i32.store8", -2, none, d_binOpList(i32, Store_u8),
         Compile_Load_Store),  // 0x3a
    M3OP("i32.store16", -2, none, d_binOpList(i32, Store_i16),
         Compile_Load_Store),  // 0x3b

    M3OP("i64.store8", -2, none, d_binOpList(i64, Store_u8),
         Compile_Load_Store),  // 0x3c
    M3OP("i64.store16", -2, none, d_binOpList(i64, Store_i16),
         Compile_Load_Store),  // 0x3d
    M3OP("i64.store32", -2, none, d_binOpList(i64, Store_i32),
         Compile_Load_Store),  // 0x3e

    M3OP("memory.size", 1, i_32, d_logOp(MemSize),
         Compile_Memory_Size),  // 0x3f
    M3OP("memory.grow", 1, i_32, d_logOp(MemGrow),
         Compile_Memory_Grow),  // 0x40
    ...
    M3OP("i32.ne", -1, i_32, d_commutativeBinOpList(i32, NotEqual),
         NULL),                                                       // 0x47
    ...
    M3OP("i32.le_s", -1, i_32, d_binOpList(i32, LessThanOrEqual),
         NULL),  // 0x4c
    M3OP("i32.le_u", -1, i_32, d_binOpList(u32, LessThanOrEqual),
         NULL),  // 0x4d
    M3OP("i32.ge_s", -1, i_32, d_binOpList(i32, GreaterThanOrEqual),
         NULL),  // 0x4e
    M3OP("i32.ge_u", -1, i_32, d_binOpList(u32, GreaterThanOrEqual),
         NULL),  // 0x4f

    M3OP("i64.eqz", 0, i_32, d_unaryOpList(i64, EqualToZero), NULL),     // 0x50
    M3OP("i64.eq", -1, i_32, d_commutativeBinOpList(i64, Equal), NULL),  // 0x51
    M3OP("i64.ne", -1, i_32, d_commutativeBinOpList(i64, NotEqual),
         NULL),                                                       // 0x52
    ...
    M3OP("i64.le_s", -1, i_32, d_binOpList(i64, LessThanOrEqual),
         NULL),  // 0x57
    M3OP("i64.le_u", -1, i_32, d_binOpList(u64, LessThanOrEqual),
         NULL),  // 0x58
    M3OP("i64.ge_s", -1, i_32, d_binOpList(i64, GreaterThanOrEqual),
         NULL),  // 0x59
    M3OP("i64.ge_u", -1, i_32, d_binOpList(u64, GreaterThanOrEqual),
         NULL),  // 0x5a

    M3OP_F("f32.eq", -1, i_32, d_commutativeBinOpList(f32, Equal),
           NULL),  // 0x5b
    M3OP_F("f32.ne", -1, i_32, d_commutativeBinOpList(f32, NotEqual),
           NULL),                                                     // 0x5c
    M3OP_F("f32.lt", -1, i_32, d_binOpList(f32, LessThan), NULL),     // 0x5d
    M3OP_F("f32.gt", -1, i_32, d_binOpList(f32, GreaterThan), NULL),  // 0x5e
    M3OP_F("f32.le", -1, i_32, d_binOpList(f32, LessThanOrEqual),
           NULL),  // 0x5f
    M3OP_F("f32.ge", -1, i_32, d_binOpList(f32, GreaterThanOrEqual),
           NULL),  // 0x60

    M3OP_F("f64.eq", -1, i_32, d_commutativeBinOpList(f64, Equal),
           NULL),  // 0x61
    M3OP_F("f64.ne", -1, i_32, d_commutativeBinOpList(f64, NotEqual),
           NULL),                                                     // 0x62
    M3OP_F("f64.lt", -1, i_32, d_binOpList(f64, LessThan), NULL),     // 0x63
    M3OP_F("f64.gt", -1, i_32, d_binOpList(f64, GreaterThan), NULL),  // 0x64
    M3OP_F("f64.le", -1, i_32, d_binOpList(f64, LessThanOrEqual),
           NULL),  // 0x65
    M3OP_F("f64.ge", -1, i_32, d_binOpList(f64, GreaterThanOrEqual),
           NULL),  // 0x66
    ...
    M3OP("i32.mul", -1, i_32, d_commutativeBinOpList(i32, Multiply),
         NULL),                                                         // 0x6c
    ...
    M3OP("i64.mul", -1, i_64, d_commutativeBinOpList(i64, Multiply),
         NULL),                                                         // 0x7e
    ...
    M3OP_F("f32.add", -1, f_32, d_commutativeBinOpList(f32, Add),
           NULL),                                                   // 0x92
    M3OP_F("f32.sub", -1, f_32, d_binOpList(f32, Subtract), NULL),  // 0x93
    M3OP_F("f32.mul", -1, f_32, d_commutativeBinOpList(f32, Multiply),
           NULL),                                                 // 0x94
    M3OP_F("f32.div", -1, f_32, d_binOpList(f32, Divide), NULL),  // 0x95
    M3OP_F("f32.min", -1, f_32, d_commutativeBinOpList(f32, Min),
           NULL),  // 0x96
    M3OP_F("f32.max", -1, f_32, d_commutativeBinOpList(f32, Max),
           NULL),                                                        // 0x97
    ...
    M3OP_F("f64.add", -1, f_64, d_commutativeBinOpList(f64, Add),
           NULL),                                                   // 0xa0
    M3OP_F("f64.sub", -1, f_64, d_binOpList(f64, Subtract), NULL),  // 0xa1
    M3OP_F("f64.mul", -1, f_64, d_commutativeBinOpList(f64, Multiply),
           NULL),                                                 // 0xa2
    M3OP_F("f64.div", -1, f_64, d_binOpList(f64, Divide), NULL),  // 0xa3
    M3OP_F("f64.min", -1, f_64, d_commutativeBinOpList(f64, Min),
           NULL),  // 0xa4
    M3OP_F("f64.max", -1, f_64, d_commutativeBinOpList(f64, Max),
           NULL),                                                        // 0xa5
    ...
    M3OP_F("i32.trunc_s/f32", 0, i_32, d_convertOpList(i32_Trunc_f32),
           Compile_Convert),  // 0xa8
    M3OP_F("i32.trunc_u/f32", 0, i_32, d_convertOpList(u32_Trunc_f32),
           Compile_Convert),  // 0xa9
    M3OP_F("i32.trunc_s/f64", 0, i_32, d_convertOpList(i32_Trunc_f64),
           Compile_Convert),  // 0xaa
    M3OP_F("i32.trunc_u/f64", 0, i_32, d_convertOpList(u32_Trunc_f64),
           Compile_Convert),  // 0xab

    M3OP("i64.extend_s/i32", 0, i_64, d_unaryOpList(i64, Extend_i32),
         NULL),  // 0xac
    M3OP("i64.extend_u/i32", 0, i_64, d_unaryOpList(i64, Extend_u32),
         NULL),  // 0xad

    M3OP_F("i64.trunc_s/f32", 0, i_64, d_convertOpList(i64_Trunc_f32),
           Compile_Convert),  // 0xae
    M3OP_F("i64.trunc_u/f32", 0, i_64, d_convertOpList(u64_Trunc_f32),
           Compile_Convert),  // 0xaf
    M3OP_F("i64.trunc_s/f64", 0, i_64, d_convertOpList(i64_Trunc_f64),
           Compile_Convert),  // 0xb0
    M3OP_F("i64.trunc_u/f64", 0, i_64, d_convertOpList(u64_Trunc_f64),
           Compile_Convert),  // 0xb1

    M3OP_F("f32.convert_s/i32", 0, f_32, d_convertOpList(f32_Convert_i32),
           Compile_Convert),  // 0xb2
    M3OP_F("f32.convert_u/i32", 0, f_32, d_convertOpList(f32_Convert_u32),
           Compile_Convert),  // 0xb3
    M3OP_F("f32.convert_s/i64", 0, f_32, d_convertOpList(f32_Convert_i64),
           Compile_Convert),  // 0xb4
    M3OP_F("f32.convert_u/i64", 0, f_32, d_convertOpList(f32_Convert_u64),
           Compile_Convert),  // 0xb5

    M3OP_F("f32.demote/f64", 0, f_32, d_unaryOpList(f32, Demote_f64),
           NULL),  // 0xb6

    M3OP_F("f64.convert_s/i32", 0, f_64, d_convertOpList(f64_Convert_i32),
           Compile_Convert),  // 0xb7
    M3OP_F("f64.convert_u/i32", 0, f_64, d_convertOpList(f64_Convert_u32),
           Compile_Convert),  // 0xb8
    M3OP_F("f64.convert_s/i64", 0, f_64, d_convertOpList(f64_Convert_i64),
           Compile_Convert),  // 0xb9
    M3OP_F("f64.convert_u/i64", 0, f_64, d_convertOpList(f64_Convert_u64),
           Compile_Convert),  // 0xba

    M3OP_F("f64.promote/f32", 0, f_64, d_unaryOpList(f64, Promote_f32),
           NULL),  // 0xbb

    M3OP_F("i32.reinterpret/f32", 0, i_32, d_convertOpList(i32_Reinterpret_f32),
           Compile_Convert),  // 0xbc
    M3OP_F("i64.reinterpret/f64", 0, i_64, d_convertOpList(i64_Reinterpret_f64),
           Compile_Convert),  // 0xbd
    M3OP_F("f32.reinterpret/i32", 0, f_32, d_convertOpList(f32_Reinterpret_i32),
           Compile_Convert),  // 0xbe
    M3OP_F("f64.reinterpret/i64", 0, f_64, d_convertOpList(f64_Reinterpret_i64),
           Compile_Convert),  // 0xbf

    M3OP("i32.extend8_s", 0, i_32, d_unaryOpList(i32, Extend8_s),
         NULL),  // 0xc0
    M3OP("i32.extend16_s", 0, i_32, d_unaryOpList(i32, Extend16_s),
         NULL),  // 0xc1
    M3OP("i64.extend8_s", 0, i_64, d_unaryOpList(i64, Extend8_s),
         NULL),  // 0xc2
    M3OP("i64.extend16_s", 0, i_64, d_unaryOpList(i64, Extend16_s),
         NULL),  // 0xc3
    M3OP("i64.extend32_s", 0, i_64, d_unaryOpList(i64, Extend32_s),
         NULL),  // 0xc4
...
#define d_m3DebugTypedOp(OP) M3OP(#OP, 0, none, {op_##OP##_i32, op_##OP##_i64})
    ...
    d_m3DebugOp(GetGlobal_s32),
    d_m3DebugOp(GetGlobal_s64),
    d_m3DebugOp(ContinueLoop),
    d_m3DebugOp(ContinueLoopIf),

    d_m3DebugOp(CopySlot_32),
    d_m3DebugOp(PreserveCopySlot_32),
    d_m3DebugOp(If_s),
    d_m3DebugOp(BranchIfPrologue_s),
    d_m3DebugOp(CopySlot_64),
    d_m3DebugOp(PreserveCopySlot_64),
    d_m3DebugOp(If_r),
    d_m3DebugOp(BranchIfPrologue_r),

    d_m3DebugOp(Select_i32_rss),
    d_m3DebugOp(Select_i32_srs),
    d_m3DebugOp(Select_i32_ssr),
    d_m3DebugOp(Select_i32_sss),
    d_m3DebugOp(Select_i64_rss),
    d_m3DebugOp(Select_i64_srs),
    d_m3DebugOp(Select_i64_ssr),
    d_m3DebugOp(Select_i64_sss),

#if d_m3HasFloat
    d_m3DebugOp(Select_f32_sss),
    d_m3DebugOp(Select_f32_srs),
    d_m3DebugOp(Select_f32_ssr),
    d_m3DebugOp(Select_f32_rss),
    d_m3DebugOp(Select_f32_rrs),
    d_m3DebugOp(Select_f32_rsr),

    d_m3DebugOp(Select_f64_sss),
    d_m3DebugOp(Select_f64_srs),
    d_m3DebugOp(Select_f64_ssr),
    d_m3DebugOp(Select_f64_rss),
    d_m3DebugOp(Select_f64_rrs),
    d_m3DebugOp(Select_f64_rsr),
#endif

    d_m3DebugOp(MemFill),
    d_m3DebugOp(MemCopy),

    d_m3DebugTypedOp(SetGlobal),
    d_m3DebugOp(SetGlobal_s32),
    d_m3DebugOp(SetGlobal_s64),

    d_m3DebugTypedOp(SetRegister),
    d_m3DebugTypedOp(SetSlot),
    d_m3DebugTypedOp(PreserveSetSlot),
#endif

#if d_m3CascadedOpcodes
    [c_waOp_extended] = M3OP("0xFC", 0, c_m3Type_unknown, d_emptyOpList,
...
const M3OpInfo c_operationsFC[] = {
    M3OP_F("i32.trunc_s:sat/f32", 0, i_32, d_convertOpList(i32_TruncSat_f32),
           Compile_Convert),  // 0x00
    M3OP_F("i32.trunc_u:sat/f32", 0, i_32, d_convertOpList(u32_TruncSat_f32),
           Compile_Convert),  // 0x01
    M3OP_F("i32.trunc_s:sat/f64", 0, i_32, d_convertOpList(i32_TruncSat_f64),
           Compile_Convert),  // 0x02
    M3OP_F("i32.trunc_u:sat/f64", 0, i_32, d_convertOpList(u32_TruncSat_f64),
           Compile_Convert),  // 0x03
    M3OP_F("i64.trunc_s:sat/f32", 0, i_64, d_convertOpList(i64_TruncSat_f32),
           Compile_Convert),  // 0x04
    M3OP_F("i64.trunc_u:sat/f32", 0, i_64, d_convertOpList(u64_TruncSat_f32),
           Compile_Convert),  // 0x05
    M3OP_F("i64.trunc_s:sat/f64", 0, i_64, d_convertOpList(i64_TruncSat_f64),
           Compile_Convert),  // 0x06
    M3OP_F("i64.trunc_u:sat/f64", 0, i_64, d_convertOpList(u64_TruncSat_f64),
           Compile_Convert),  // 0x07

    M3OP_RESERVED,
    M3OP_RESERVED,

    M3OP("memory.copy", 0, none, d_emptyOpList,
         Compile_Memory_CopyFill),  // 0x0a
    M3OP("memory.fill", 0, none, d_emptyOpList,
         Compile_Memory_CopyFill),  // 0x0b

    M3OP_RESERVED,
    M3OP_RESERVED,
    M3OP_RESERVED,
    M3OP_RESERVED,  // 0x0c...0x0f
    M3OP("table.size", 0, i_32, d_emptyOpList, Compile_Table_Size),  // 0x10
...
M3Result CompileBlockStatements(IM3Compilation o) {
  ...
  while (o->wasm < o->wasmEnd) {
    ...
    if (opinfo == NULL)
      _throw(ErrorCompile(m3Err_unknownOpcode, o, "opcode '%x' not available",
                          opcode));
...
M3Result CompileBlock(IM3Compilation o, IM3FuncType i_blockType,
  ...
  /*
   The block stack frame is a little strange but for good reasons.  Because
  blocks need to be restarted to compile different pathways (if/else), the
  incoming params must be saved.  The parameters are popped and validated.  But,
  then the stack top is readjusted so they aren't subsequently overwritten.
   Next, the result are preallocated to find destination slots.  But again these
  are immediately popped (deallocated) and the stack top is readjusted to keep
  these records in pace. This allows branch instructions to find their result
  landing pads.  Finally, the params are copied from the "dead" records and
  pushed back onto the stack as active stack items for the
  CompileBlockStatements () call.
  ...
  */
  ...
  _try {
    ...
    if (o->function)  // skip for expressions
    {
      ...
      if (not((i_blockOpcode == c_waOp_if and numResults) or
              o->previousOpcode == c_waOp_else)) {
...
static M3Result CompileLocals(IM3Compilation o) {
  ...
  for (u32 l = 0; l < numLocalBlocks; ++l) {
    ...
    numLocals += varCount;
    m3log(compile, "pushing locals. count: %d; type: %s", varCount,
          c_waTypes[localType]);
...
static M3Result ReserveConstants(IM3Compilation o) {
  ...
  // in the interest of speed, this blindly scans the Wasm code looking for any
  // byte that looks like an const opcode.
  ...
  // if constants overflow their reserved stack space, the compiler simply emits
  // op_Const operations as needed. Compiled expressions (global inits) don't
  // pass through this ReserveConstants function and thus always produce inline
  // constants.
...
M3Result CompileFunction(IM3Function io_function) {
  ...
  m3log(compile, "compiling: [%d] %s %s; wasm-size: %d", io_function->index,
        m3_GetFunctionName(io_function), SPrintFuncTypeSignature(funcType),
  ...
  d_m3Assert(
      ...
      d_m3MaxFunctionStackHeight *
          (d_m3Use32BitSlots + 1))  // need twice as many slots in 32-bit mode
  ...
  _try {
    ...
    for (u16 i = 0; i < numRetSlots; ++i) MarkSlotAllocated(o, i);
    ...
    o->slotMaxAllocatedIndexPlusOne = o->function->numRetAndArgSlots =
    ...
    // start tracking the max stack used (Push() also updates this value) so
    // that op_Entry can precisely detect stack overflow
    o->maxStackSlots = o->slotMaxAllocatedIndexPlusOne =
    ...
    if (numConstantSlots) {
      io_function->constants =
          m3_CopyMem(o->constants, io_function->numConstantBytes);

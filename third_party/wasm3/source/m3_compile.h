...
typedef struct {
  ...
  u16 stackFirstDynamicIndex;  // args and locals are pushed to the stack so
                               // that their slot locations can be tracked. the
                               // wasm model itself doesn't treat these values
                               // as being on the stack, so
                               // stackFirstDynamicIndex marks the start of the
                               // real Wasm stack
  u16 stackIndex;              // current stack top

  u16 slotFirstConstIndex;
  u16 slotMaxConstIndex;  // as const's are encountered during compilation this
                          // tracks their location in the "real" stack

  u16 slotFirstLocalIndex;
  u16 slotFirstDynamicIndex;  // numArgs + numLocals + numReservedConstants. the
                              // first mutable slot available to the compiler.
...
typedef struct M3OpInfo {
  ...
  // for most operations:
  // [0]= top operand in register, [1]= top operand in stack, [2]= both operands
  // in stack
...
#define M3OP_F(n, o, t, op, ...)                                         \
  M3OP(n, o, t,                                                          \
       {op_Unsupported, op_Unsupported, op_Unsupported, op_Unsupported}, \
       __VA_ARGS__)
#else
#define M3OP_F(...) \
  { 0 }

...
void m3_PrintM3Info() {
  ...
  //  (M3CodePage), c_m3CodePageNumSlots);
...
// TODO: these 'static char string []' aren't thread-friendly.  though these
// functions are mainly for simple diagnostics during development, it'd be nice
// if they were fully reliable.
...
size_t SPrintArg(char *o_string, size_t i_stringBufferSize, voidptr_t i_sp,
                 u8 i_type) {
  int len = 0;
...
#define d_m3Decoder(FUNC)                                                   \
  void Decode_##FUNC(char *o_string, u8 i_opcode, IM3Operation i_operation, \
                     IM3OpInfo i_opInfo, pc_t *o_pc)
...
void DecodeOperation(char *o_string, u8 i_opcode, IM3Operation i_operation,
                     IM3OpInfo i_opInfo, pc_t *o_pc) {
#define d_m3Decode(OPCODE, FUNC)                                    \
  ...
  switch (i_opcode) {
    ...
    d_m3Decode(0xc5, Entry) d_m3Decode(c_waOp_call, Call)
...
// WARNING/TODO: this isn't fully implemented. it blindly assumes each word is a
// Operation pointer and, if an operation happens to missing from the
// c_operations table it won't be recognized here
void dump_code_page(IM3CodePage i_codePage, pc_t i_startPC) {
  ...
  m3log(code, "free-lines: %d",
        i_codePage->info.numLines - i_codePage->info.lineIndex);
}

void dump_type_stack(IM3Compilation o) {
  /* Reminders about how the stack works! :)
   -- args & locals remain on the type stack for duration of the function.
   Denoted with a constant 'A' and 'L' in this dump.
   -- the initial stack dumps originate from the CompileLocals () function, so
   these identifiers won't/can't be applied until this compilation stage is
   finished
   -- constants are not statically represented in the type stack (like args &
   constants) since they don't have/need write counts

   -- the number shown for static args and locals (value in wasmStack [i])
   represents the write count for the variable

   -- (does Wasm ever write to an arg? I dunno/don't remember.)
   -- the number for the dynamic stack values represents the slot number.
   -- if the slot index points to arg, local or constant it's denoted with a
   lowercase 'a', 'l' or 'c'

   */

  // for the assert at end of dump:
  i32 regAllocated[2] = {(i32)IsRegisterAllocated(o, 0),
                         (i32)IsRegisterAllocated(o, 1)};

  // display whether r0 or fp0 is allocated. these should then also be reflected
  // somewhere in the stack too.
  d_m3Log(stack, "\n");
  d_m3Log(stack, "        ");
  printf("%s %s    ", regAllocated[0] ? "(r0)" : "    ",
         regAllocated[1] ? "(fp0)" : "     ");
  printf("\n");
  ...
  for (u32 p = 1; p <= 2; ++p) {
    ...
    for (u32 i = 0; i < o->stackIndex; ++i) {
      if (i > 0 and i == o->stackFirstDynamicIndex) printf("#");
      // <Primjs begin>
      if (i == (u32)o->block.blockStackIndex) printf(">");
      // <Primjs end>
      ...
      if (p == 1) {
        ...
        while (strlen(item) < s) strcat(item, " ");
      }
  ...
  //    for (u32 r = 0; r < 2; ++r)
  //        d_m3Assert (regAllocated [r] == 0);         // reg allocation &
  //        stack out of sync
  ...
  if (maxSlot > o->slotFirstDynamicIndex) {
    ...
    for (u16 i = o->slotFirstDynamicIndex; i < maxSlot; ++i) printf("----");

    printf("\n");

    d_m3Log(stack, "                 slot |");
    for (u16 i = o->slotFirstDynamicIndex; i < maxSlot; ++i) printf("%3d|", i);
...
static const char *GetOpcodeIndentionString(i32 blockDepth) {
  ...
  indent -= (blockDepth * 2);
  if (indent < s_spaces) indent = s_spaces;
...
void log_opcode(IM3Compilation o, m3opcode_t i_opcode) {
  ...
  if (i_opcode == c_waOp_end or i_opcode == c_waOp_else) depth--;

  m3log(compile, "%4d | 0x%02x  %s %s", o->numOpcodes++, i_opcode,
        GetOpcodeIndentionString(depth), GetOpInfo(i_opcode)->name);
}
...
void m3_PrintProfilerInfo() {
  ...
  do {
    ...
    for (u32 i = 0; i <= d_m3ProfilerSlotMask; ++i) {
      ...
      if (slot->opName) {
        if (slot->hitCount > maxSlot->hitCount) maxSlot = slot;
      }
    }

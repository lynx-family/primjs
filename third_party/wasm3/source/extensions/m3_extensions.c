...
M3Result m3_InjectFunction(IM3Module i_module, int32_t* io_functionIndex,
  ...
  if (index >= 0) {
    _throwif("function index out of bounds",
    ...
    i_module->funcTypes =
        m3_ReallocArray(IM3FuncType, i_module->funcTypes,
                        i_module->numFuncTypes, funcTypeIndex);
  ...
  if (function->ownsWasmCode) m3_Free(function->wasm);

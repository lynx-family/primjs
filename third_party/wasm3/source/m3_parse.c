...
M3Result ParseType_Table(M3TableInfo* o_table, bytes_t* io_bytes,
                         cbytes_t i_end) {
  M3Result result = m3Err_none;

  u8 flag;

  _(ReadLEB_u32(&o_table->elemTy, io_bytes, i_end));
  m3log(parse, "** Type [%d]", elemType);

  d_m3Assert(o_table->elemTy == kFuncRef || o_table->elemTy == kExternRef);

  _(ReadLEB_u7(&flag, io_bytes, i_end));  // really a u1
  _(ReadLEB_u32(&o_table->curSize, io_bytes, i_end));

  o_table->maxSize = d_m3MaxSaneTableSize;
  if (flag) _(ReadLEB_u32(&o_table->maxSize, io_bytes, i_end));
  ...
  for (u32 i = 0; i < numImports; ++i) {
    ...
    switch (importKind) {
      ...
      case d_externalKind_table:
        // <Primjs begin>
        _(ParseType_Table(&io_module->tableInfo, &i_bytes, i_end));
        io_module->tableImportInfo = m3_AllocStruct(M3ImportInfo);
        *(io_module->tableImportInfo) = import;
        import = clearImport;
        // <Primjs end>
        break;
      ...
      case d_externalKind_memory: {
        ...
        io_module->memoryImportInfo = m3_AllocStruct(M3ImportInfo);
        *(io_module->memoryImportInfo) = import;
        import = clearImport;
      } break;
...
M3Result ParseSection_Export(IM3Module io_module, bytes_t i_bytes,
  ...
  // <Primjs begin>
  io_module->exportedFuncs =
      m3_Malloc("ExportedFunc", sizeof(M3ExportedFunction));
  M3ExportedFunction* cur = io_module->exportedFuncs;
  cur->func = NULL;
  cur->next = NULL;
  // <Primjs end>
  ...
  for (u32 i = 0; i < numExports; ++i) {
    ...
    if (exportKind == d_externalKind_function) {
      ...
      if (func->numNames < d_m3MaxDuplicateFunctionImpl) {
        ...
        // <Primjs begin>
        cur->func = func;
        cur->next = m3_Malloc("ExportedFunc", sizeof(M3ExportedFunction));
        cur = cur->next;
        cur->func = NULL;
        // <Primjs end>
      }
    ...
    // <Primjs begin>
    // support table export
    else if (exportKind == d_externalKind_table) {
      // only support one table per module in mvp
      _throwif(m3Err_wasmMalformed, index != 0);
      io_module->tableInfo.tableName = utf8;
      utf8 = NULL;  // ownership transferred to tableInfo.tableName
    } else if (exportKind == d_externalKind_memory) {
      // only support memory0 in mvp
      _throwif(m3Err_wasmMalformed, index != 0);

      size_t oldSize = (io_module->memoryNameCount) * sizeof(cstr_t);
      size_t newSize = oldSize + sizeof(cstr_t);
      void* newNames = m3_Realloc("Module Export Memories' Names",
                                  io_module->memoryNames, newSize, oldSize);
      io_module->memoryNames = (cstr_t*)newNames;
      io_module->memoryNames[io_module->memoryNameCount] = utf8;
      io_module->memoryNameCount++;

      utf8 = NULL;  // ownership transferred to memoryName
    }
    // <Primjs end>
...
M3Result ParseSection_Table(M3Module* io_module, bytes_t i_bytes,
  ...
  _throwif(m3Err_tooManyTableSections, numTables != 1);

  ParseType_Table(&io_module->tableInfo, &i_bytes, i_end);
...
M3Result ParseModuleSection(M3Module* o_module, u8 i_sectionType,
  ...
  if (i_sectionType <= 12) parser = s_parsers[i_sectionType];
...
M3Result m3_ParseModule(IM3Environment i_environment, IM3Module* o_module,
  ...
  _try {
    ...
    // After we know that module does not import a memory, malloc a M3Memory
    // wrapper for it.
    if (!module->memoryImportInfo) {
      module->memory = m3_AllocStruct(M3Memory);
      _throwifnull(module->memory);
    }
  }
_catch:

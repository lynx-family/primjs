...
#define __builtin_popcountll(x) \
  (__popcnt((x) & 0xFFFFFFFF) + __popcnt((x) >> 32))
...
#define OP_I32_TRUNC_F32(RES, A) \
  OP_TRUNC(RES, A, i32, -2147483904.0f, 2147483648.0f)
#define OP_U32_TRUNC_F32(RES, A) OP_TRUNC(RES, A, u32, -1.0f, 4294967296.0f)
#define OP_I32_TRUNC_F64(RES, A) \
  OP_TRUNC(RES, A, i32, -2147483649.0, 2147483648.0)
#define OP_U32_TRUNC_F64(RES, A) OP_TRUNC(RES, A, u32, -1.0, 4294967296.0)

#define OP_I64_TRUNC_F32(RES, A) \
  OP_TRUNC(RES, A, i64, -9223373136366403584.0f, 9223372036854775808.0f)
#define OP_U64_TRUNC_F32(RES, A) \
  OP_TRUNC(RES, A, u64, -1.0f, 18446744073709551616.0f)
#define OP_I64_TRUNC_F64(RES, A) \
  OP_TRUNC(RES, A, i64, -9223372036854777856.0, 9223372036854775808.0)
#define OP_U64_TRUNC_F64(RES, A) \
  OP_TRUNC(RES, A, u64, -1.0, 18446744073709551616.0)
...
#define OP_I32_TRUNC_SAT_F32(RES, A) \
  OP_TRUNC_SAT(RES, A, i32, -2147483904.0f, 2147483648.0f, INT32_MIN, INT32_MAX)
#define OP_U32_TRUNC_SAT_F32(RES, A) \
  OP_TRUNC_SAT(RES, A, u32, -1.0f, 4294967296.0f, 0UL, UINT32_MAX)
#define OP_I32_TRUNC_SAT_F64(RES, A) \
  OP_TRUNC_SAT(RES, A, i32, -2147483649.0, 2147483648.0, INT32_MIN, INT32_MAX)
#define OP_U32_TRUNC_SAT_F64(RES, A) \
  OP_TRUNC_SAT(RES, A, u32, -1.0, 4294967296.0, 0UL, UINT32_MAX)

#define OP_I64_TRUNC_SAT_F32(RES, A)                                         \
  OP_TRUNC_SAT(RES, A, i64, -9223373136366403584.0f, 9223372036854775808.0f, \
               INT64_MIN, INT64_MAX)
#define OP_U64_TRUNC_SAT_F32(RES, A) \
  OP_TRUNC_SAT(RES, A, u64, -1.0f, 18446744073709551616.0f, 0ULL, UINT64_MAX)
#define OP_I64_TRUNC_SAT_F64(RES, A)                                       \
  OP_TRUNC_SAT(RES, A, i64, -9223372036854777856.0, 9223372036854775808.0, \
               INT64_MIN, INT64_MAX)
#define OP_U64_TRUNC_SAT_F64(RES, A) \
  OP_TRUNC_SAT(RES, A, u64, -1.0, 18446744073709551616.0, 0ULL, UINT64_MAX)

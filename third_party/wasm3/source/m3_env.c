...
static M3Result InitMemory(IM3Runtime io_runtime, IM3Module i_module) {
  ...
  if (not i_module->memoryImportInfo) {
    u32 maxPages = i_module->memoryInfo.maxPages;
    i_module->memory->maxPages = maxPages ? maxPages : 65536;

    result = ResizeMemory(i_module->memory, io_runtime,
                          i_module->memoryInfo.initPages);

    // Link memory with module when loading Module
    m3_LinkMemory(i_module, i_module->memory);
  }

  return result;
}

M3Result InitTable(IM3Runtime io_runtime, IM3Module i_module) {
  M3Result result = m3Err_none;

  if (not i_module->tableImportInfo) {
    // FIXME: support externref.
    i_module->tableElems =
        m3_AllocArray(IM3Function, i_module->tableInfo.curSize);
    if (i_module->tableElems == NULL) {
      result = m3Err_mallocFailed;
    }
  } else {
    if (not i_module->table) {
      result = m3Err_tableImportMissing;
    }
  }
...
M3Result ResizeMemory(IM3Memory memory, IM3Runtime io_runtime, u32 i_numPages) {
  M3Result result = m3Err_none;
...
// Malloc a linear memory here, this memory do not belong to any module or
// runtime at this time.
M3Result AllocMemory(IM3Memory memory, IM3Runtime io_runtime,
                     u32 numPagesToAlloc) {
  M3Result result = m3Err_none;
  ...
  if (numPreviousBytes) numPreviousBytes += sizeof(M3MemoryHeader);

  M3ModuleList *moduleList =
      memory->mallocated ? memory->mallocated->moduleList : NULL;

  void *newMem = m3_Realloc("Wasm Linear Memory", memory->mallocated, numBytes,
                            numPreviousBytes);
  _throwifnull(newMem);

  memory->mallocated = (M3MemoryHeader *)newMem;
  memory->mallocated->moduleList = moduleList;
    ...
    //          if (io_module->globalMemory)
    {
      for (u32 i = 0; i < io_module->numGlobals; ++i) {
        ...
        if (g->initExpr) {
          ...
          result = EvaluateExpression(io_module, &g->intValue, g->type, &start,
...
M3Result InitDataSegments(M3Memory *io_memory, IM3Module io_module) {
  ...
  _throwif(m3Err_unallocatedMemory, !io_memory || !(io_memory->mallocated));
...
M3Result InitElements(IM3Module io_module) {
  ...
  for (u32 i = 0; i < io_module->numElementSegments; ++i) {
    ...
    if (index == 0) {
      ...
      _throwif("table overflow", endElement > io_module->tableInfo.maxSize);

      // is there any requirement that elements must be in increasing sequence?
      // make sure the table isn't shrunk.
      if (endElement > io_module->tableInfo.curSize) {
        io_module->tableElems =
            m3_ReallocArray(IM3Function, io_module->tableElems, endElement,
                            io_module->tableInfo.curSize);
        io_module->tableInfo.curSize = (u32)endElement;
      }
      _throwifnull(io_module->tableElems);
      ...
      for (u32 e = 0; e < numElements; ++e) {
        ...
        io_module->tableElems[e + offset] = function;
      }
    } else
...
M3Result m3_RunStart(IM3Module io_module) {
  ...
  if (io_module and io_module->startFunction >= 0) {
    ...
    result =
        (M3Result)RunCode(function->compiled, (m3stack_t)runtime->stack,
                          io_module->memory->mallocated, d_m3OpDefaultArgs);
...
M3Result m3_LoadModule(IM3Runtime io_runtime, IM3Module io_module) {
  ...
  M3Memory *memory = io_module->memory;

  _(InitTable(io_runtime, io_module));
...
// Must call after m3_FreeModule if multi modules in M3Runtime
void m3_UnloadModule(IM3Module io_module) {
  if (M3_UNLIKELY(!io_module || !io_module->runtime)) {
    return;
  }

  IM3Runtime runtime = io_module->runtime;
  IM3Module *cur = &runtime->modules;
...
M3Result m3_GetGlobal(IM3Global i_global, IM3TaggedValue o_value) {
  ...
  switch (i_global->type) {
    case c_m3Type_i32:
      o_value->value.i32 = i_global->intValue;
      break;
    case c_m3Type_i64:
      o_value->value.i64 = i_global->intValue;
      break;
#if d_m3HasFloat
    case c_m3Type_f32:
...
M3Result m3_SetGlobal(IM3Global i_global, const IM3TaggedValue i_value) {
  ...
  switch (i_value->type) {
    case c_m3Type_i32:
      i_global->intValue = i_value->value.i32;
      break;
    case c_m3Type_i64:
      i_global->intValue = i_value->value.i64;
      break;
#if d_m3HasFloat
    case c_m3Type_f32:
...
void *v_FindFunction(IM3Module i_module, const char *const i_name) {
  for (u32 i = 0; i < i_module->numFunctions; ++i) {
    ...
    for (int j = 0; j < f->numNames; j++) {
      if (f->names[j] and strcmp(f->names[j], i_name) == 0) return f;
    }
  }
...
M3Result m3_CallVL(IM3Function i_function, va_list i_args) {
  ...
  result = (M3Result)RunCode(i_function->compiled, (m3stack_t)(runtime->stack),
                             i_function->module->memory->mallocated,
                             d_m3OpDefaultArgs);
...
IM3CodePage AcquireCodePageWithCapacity(IM3Runtime i_runtime,
  ...
  if (not page) {
    ...
    if (not page) page = NewCodePage(i_runtime, i_minLineCount);

    if (page) i_runtime->numCodePages++;
  }
...
uint8_t *m3_GetMemory(IM3Memory i_memory, uint32_t *o_memorySizeInBytes,
  ...
  if (i_memory) {
    u32 size = (u32)i_memory->mallocated->length;
    if (size) {
      buf = m3MemData(i_memory->mallocated);
    }
...
uint32_t m3_GetMemorySize(IM3Memory i_memory) {
  return i_memory->mallocated->length;
}

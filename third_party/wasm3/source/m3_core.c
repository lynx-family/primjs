...
M3_WEAK
M3Result m3_Yield() { return m3Err_none; }
...
#define HEAP_ALIGN_PTR(P)                              \
  P = (u8*)(((size_t)(P) + (d_m3FixedHeapAlign - 1)) & \
            ~(d_m3FixedHeapAlign - 1));
#else
#define HEAP_ALIGN_PTR(P)
#endif
...
void* m3_Malloc_Impl(size_t i_size) { return calloc(i_size, 1); }

void m3_Free_Impl(void* io_ptr) { free(io_ptr); }
...
int m3StackGetMax() { return stack_start - stack_end; }
...
bool Is64BitType(u8 i_m3Type) {
  if (i_m3Type == c_m3Type_i64 or i_m3Type == c_m3Type_f64)
    return true;
  else if (i_m3Type == c_m3Type_i32 or i_m3Type == c_m3Type_f32 or
           i_m3Type == c_m3Type_none)
    return false;
  else
...
u32 SizeOfType(u8 i_m3Type) {
  if (i_m3Type == c_m3Type_i32 or i_m3Type == c_m3Type_f32) return sizeof(i32);
...
void FillBacktraceFunctionInfo(IM3Runtime io_runtime, IM3Function i_function) {
  ...
  if (!io_runtime->backtrace.lastFrame) return;

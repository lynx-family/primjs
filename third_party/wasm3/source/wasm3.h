...
struct M3Table;
typedef struct M3Table *IM3Table;
...
d_m3ErrorConst(none, NULL)
    ...
    d_m3ErrorConst(incompatibleWasmVersion, "incompatible Wasm binary version")
        d_m3ErrorConst(wasmMalformed, "malformed Wasm binary") d_m3ErrorConst(
            misorderedWasmSection,
            "out of order Wasm section") d_m3ErrorConst(wasmUnderrun,
                                                        "underrun while "
                                                        "parsing Wasm binary")
            d_m3ErrorConst(wasmOverrun, "overrun while parsing Wasm binary")
                d_m3ErrorConst(wasmMissingInitExpr,
                               "missing init_expr in Wasm binary")
                    d_m3ErrorConst(lebOverflow,
                                   "LEB encoded value "
                                   "overflow")
                        d_m3ErrorConst(missingUTF8,
                            ...
                            d_m3ErrorConst(
                                ...
                                d_m3ErrorConst(
                                    ...
                                    d_m3ErrorConst(invalidTypeId,
                                                   "unknown value_type")
                                        d_m3ErrorConst(tooManyMemorySections,
                                                       "only one memory per "
                                                       "module is supported")
                                            d_m3ErrorConst(
                                                tooManyTableSections,
                                                "only one table per module is "
                                                "supported")
                                                d_m3ErrorConst(
                                                    tooManyArgsRets,
    ...
    // link errors
    d_m3ErrorConst(moduleNotLinked,
                   "attempting to use module that is not loaded")
        d_m3ErrorConst(moduleAlreadyLinked,
                       "attempting to bind module to multiple runtimes")
            d_m3ErrorConst(functionLookupFailed, "function lookup failed")
                d_m3ErrorConst(functionImportMissing,
                               "missing imported function")
                    d_m3ErrorConst(memoryImportMissing,
                                   "missing imported memory or invalid memory")
                        d_m3ErrorConst(
                            tableImportMissing,
                            "missing imported table or invalid table")

                            d_m3ErrorConst(malformedFunctionSignature,
                                           "malformed function signature")

    // compilation errors
    d_m3ErrorConst(noCompiler, "no compiler found for opcode")
        d_m3ErrorConst(unknownOpcode,
                       "unknown opcode") d_m3ErrorConst(restrictedOpcode,
                                                        "restricted opcode")
            d_m3ErrorConst(functionStackOverflow,
                           "compiling function overran its stack height limit")
                d_m3ErrorConst(functionStackUnderrun,
                               "compiling function underran the stack")
                    d_m3ErrorConst(mallocFailedCodePage,
                                   "memory allocation failed when acquiring a "
                                   "new M3 code page")
                        d_m3ErrorConst(settingImmutableGlobal,
                                       "attempting to set an immutable global")
                            d_m3ErrorConst(typeMismatch,
                                           "incorrect type on stack")
                                d_m3ErrorConst(typeCountMismatch,
    ...
    d_m3ErrorConst(missingCompiledCode, "function is missing compiled m3 code")
        d_m3ErrorConst(wasmMemoryOverflow, "runtime ran out of memory")
            d_m3ErrorConst(globalMemoryNotAllocated,
                           "global memory is missing from a module")
                d_m3ErrorConst(globaIndexOutOfBounds,
                               "global index is too large")
                    d_m3ErrorConst(argumentCountMismatch,
                                   "argument count mismatch")
                        d_m3ErrorConst(argumentTypeMismatch,
                                       "argument type mismatch")
                            d_m3ErrorConst(globalLookupFailed,
                                           "global lookup failed")
                                d_m3ErrorConst(globalTypeMismatch,
                                               "global type mismatch")
                                    d_m3ErrorConst(globalNotMutable,
                                                   "global is not mutable")
                                        d_m3ErrorConst(
                                            memoryInvalidRange,
                                            "invalid range of a memory")
                                            d_m3ErrorConst(
                                                tableInvalidRange,
                                                "invalid range of a table")

    // traps
    d_m3ErrorConst(trapOutOfBoundsTableAccess,
                   "[trap] out of bounds table access")
        d_m3ErrorConst(trapOutOfBoundsMemoryAccess,
            ...
            d_m3ErrorConst(trapDivisionByZero, "[trap] integer divide by zero")
                d_m3ErrorConst(trapIntegerOverflow, "[trap] integer overflow")
                    d_m3ErrorConst(trapIntegerConversion,
                                   "[trap] invalid conversion to integer")
                        d_m3ErrorConst(trapIndirectCallTypeMismatch,
                                       "[trap] indirect call type mismatch")
                            d_m3ErrorConst(trapTableIndexOutOfRange,
                                           "[trap] undefined element")
                                d_m3ErrorConst(trapTableElementIsNull,
                                               "[trap] null table element")
                                    d_m3ErrorConst(trapExit,
                                                   "[trap] program called exit")
                                        d_m3ErrorConst(
                                            trapAbort,
    ...
    // others
    d_m3ErrorConst(detachLinkedMemory,
                   "imported memory against the module detached")
        d_m3ErrorConst(releasedMemory, "actually released Memory's memory")
            d_m3ErrorConst(unallocatedMemory, "unallocated linear memory")

    //-------------------------------------------------------------------------------------------------------------------------------
    //  configuration, can be found in m3_config.h, m3_config_platforms.h,
    //  m3_core.h)
    //-------------------------------------------------------------------------------------------------------------------------------
...
uint8_t *m3_GetMemory(IM3Memory i_memory, uint32_t *o_memorySizeInBytes,
                      uint32_t i_memoryIndex);

// This is used internally by Raw Function helpers
uint32_t m3_GetMemorySize(IM3Memory i_memory);
...
// Only modules not loaded into a M3Runtime need to be freed. A module is
// considered unloaded if a. m3_LoadModule has not yet been called on that
// module. Or, b. m3_LoadModule returned a result.
void m3_FreeModule(IM3Module i_module);

//  LoadModule transfers ownership of a module to the runtime. Do not free
//  modules once successfully loaded into the runtime
M3Result m3_LoadModule(IM3Runtime io_runtime, IM3Module io_module);
void m3_UnloadModule(IM3Module io_module);
...
// _sp. Placeholder return value slots are first and arguments after. So, the
// first argument is at _sp [numReturns] Return values should be written into
// _sp [0] to _sp [num_returns - 1]
...
M3Result m3_NewMemory(IM3Memory *i_memory, IM3Runtime io_runtime,
...
M3Result m3_NewTable(IM3Table *i_table, M3TableElemType tableElemTy,
                     uint32_t initial, uint32_t maximum);
...
// The runtime owns the backtrace, do not free the backtrace you obtain. Returns
// NULL if there's no backtrace.
...
#define m3ApiGetArgMem(TYPE, NAME) \
  TYPE NAME = (TYPE)m3ApiOffsetToPtr(*((uint32_t *)(_sp++)));

#define m3ApiIsNullPtr(addr) ((void *)(addr) <= _mem)
#define m3ApiCheckMem(addr, len)                                   \
  {                                                                \
    if (M3_UNLIKELY(((void *)(addr) < _mem) ||                     \
                    ((uint64_t)(uintptr_t)(addr) + (len)) >        \
                        ((uint64_t)(uintptr_t)(_mem) +             \
                         (((M3MemoryHeader *)_mem - 1)->length)))) \
      m3ApiTrap(m3Err_trapOutOfBoundsMemoryAccess);                \
  }

#define m3ApiRawFunction(NAME)                                               \
  const void *NAME(IM3Runtime runtime, IM3ImportContext _ctx, uint64_t *_sp, \
                   void *_mem)

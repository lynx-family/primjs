...
void m3_FreeModule(IM3Module i_module) {
  if (M3_UNLIKELY(!i_module)) return;

  m3log(module, "freeing module: %s (funcs: %d; segments: %d)", i_module->name,
        i_module->numFunctions, i_module->numDataSegments);
  ...
  // <Primjs begin>
  if (i_module->table) {
    // Detach the imported table.
    i_module->table->mod = NULL;
    i_module->table->info.tableName = NULL;
  } else {
    // If a table is detached or no table exported/imported
    // at all, table elements array is released here.
    m3_Free(i_module->tableElems);
  }
  // Always release tableImportInfo & tableName.
  m3_Free(i_module->tableImportInfo);
  m3_Free(i_module->tableInfo.tableName);
  ...
  IM3Memory memory = i_module->memory;

  // memory could be removed only if it is not imported or exported
  if (!i_module->memoryImportInfo && !i_module->memoryNameCount)
    m3_FreeMemory(memory);
  else
    m3_UnLinkMemory(i_module, memory);

  for (uint8_t i = 0; i < i_module->memoryNameCount; i++) {
    m3_Free(i_module->memoryNames[i]);
  }
  i_module->memoryNames = NULL;
  i_module->memoryNameCount = 0;

  m3_Free(i_module->memoryImportInfo);

  M3ExportedFunction* target = i_module->exportedFuncs;
  M3ExportedFunction* temp = NULL;
  while (target) {
    temp = target->next;
    m3_Free(target);
    target = temp;
...
M3Result Module_AddGlobal(IM3Module io_module, IM3Global* o_global, u8 i_type,
                          bool i_mutable, bool i_isImported) {
  _try {
    u32 index = io_module->numGlobals++;
    io_module->globals = m3_ReallocArray(M3Global, io_module->globals,
                                         io_module->numGlobals, index);
...
M3Result Module_PreallocFunctions(IM3Module io_module, u32 i_totalFunctions) {
  _try {
    if (i_totalFunctions > io_module->allFunctions) {
      io_module->functions =
          m3_ReallocArray(M3Function, io_module->functions, i_totalFunctions,
                          io_module->allFunctions);
...
M3Result Module_AddFunction(IM3Module io_module, u32 i_typeIndex,
                            IM3ImportInfo i_importInfo) {
  _try {
    u32 index = io_module->numFunctions++;
    _(Module_PreallocFunctions(io_module, io_module->numFunctions));

    _throwif("type sig index out of bounds",
             i_typeIndex >= io_module->numFuncTypes);
...
const char* m3_GetModuleName(IM3Module i_module) {
  if (!i_module || !i_module->name) return ".unnamed";

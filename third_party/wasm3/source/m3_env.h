...
typedef struct M3ExportedFunction {
  M3Function* func;
  struct M3ExportedFunction* next;
} M3ExportedFunction;
// <Primjs end>
...
typedef struct M3Module {
  ...
  // table instance
  IM3Function* tableElems;
  M3TableInfo tableInfo;
  IM3Table table;
  IM3ImportInfo tableImportInfo;

  // memory instance
  M3MemoryInfo memoryInfo;
  IM3Memory memory;
  IM3ImportInfo memoryImportInfo;

  // support memory exports, memoryNames is null if not exported
  cstr_t* memoryNames;
  uint8_t memoryNameCount;
  ...
  // <Primjs begin>
  M3ExportedFunction* exportedFuncs;
  // <Primjs end>
} M3Module;
...
M3Result Module_AddFunction(IM3Module io_module, u32 i_typeIndex,
                            IM3ImportInfo i_importInfo /* can be null */);
...
typedef struct M3Environment {
  ...
  IM3FuncType funcTypes;  // linked list of unique M3FuncType structs that can
                          // be compared using pointer-equivalence

  IM3FuncType
      retFuncTypes[c_m3Type_unknown];  // these 'point' to elements in the
                                       // linked list above. the number of
                                       // elements must match the basic types as
                                       // per M3ValueType
...
// takes ownership of io_funcType and returns a pointer to the persistent
// version (could be same or different)
...
typedef struct M3Runtime {
  ...
  M3CodePage*
      pagesOpen;  // linked list of code pages with writable space on them

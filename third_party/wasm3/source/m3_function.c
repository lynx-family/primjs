...
M3Result AllocFuncType(IM3FuncType *o_functionType, u32 i_numTypes) {
  *o_functionType =
      (IM3FuncType)m3_Malloc("M3FuncType", sizeof(M3FuncType) + i_numTypes);
  return (*o_functionType) ? m3Err_none : m3Err_mallocFailed;
}

bool AreFuncTypesEqual(const IM3FuncType i_typeA, const IM3FuncType i_typeB) {
  if (i_typeA->numRets == i_typeB->numRets &&
      i_typeA->numArgs == i_typeB->numArgs) {
    return (memcmp(i_typeA->types, i_typeB->types,
                   i_typeA->numRets + i_typeA->numArgs) == 0);
  }
...
void Function_Release(IM3Function i_function) {
  ...
  if (i_function->ownsWasmCode) m3_Free(i_function->wasm);
...
cstr_t *GetFunctionNames(IM3Function i_function, u16 *o_numNames) {
  if (!i_function || !o_numNames) return NULL;
...
u16 GetFunctionNumArgs(IM3Function i_function) {
  ...
  if (i_function) {
    if (i_function->funcType) numArgs = i_function->funcType->numArgs;
  }
...
u16 GetFunctionNumReturns(IM3Function i_function) {
  ...
  if (i_function) {
    if (i_function->funcType) numReturns = i_function->funcType->numRets;
  }
...
u8 GetFunctionReturnType(const IM3Function i_function, u16 i_index) {
  return i_function ? GetFuncTypeResultType(i_function->funcType, i_index)
                    : c_m3Type_unknown;
}

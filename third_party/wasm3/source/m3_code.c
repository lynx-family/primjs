...
IM3CodePage NewCodePage(IM3Runtime i_runtime, u32 i_minNumLines) {
  ...
  pageSize = (pageSize + (d_m3CodePageAlignSize - 1)) &
             ~(d_m3CodePageAlignSize - 1);  // align
  ...
  if (page) {
    ...
    u32 pageSizeBt = sizeof(M3CodeMappingPage) +
                     sizeof(M3CodeMapEntry) * page->info.numLines;
    page->info.mapping =
        (M3CodeMappingPage *)m3_Malloc("M3CodeMappingPage", pageSizeBt);
    ...
    m3log(runtime, "new page: %p; seq: %d; bytes: %d; lines: %d",
          GetPagePC(page), page->info.sequence, pageSize, page->info.numLines);
  }
...
void FreeCodePages(IM3CodePage *io_list) {
  ...
  while (page) {
    m3log(code, "free page: %d; %p; util: %3.1f%%", page->info.sequence, page,
...
pc_t GetPageStartPC(IM3CodePage i_page) { return &i_page->code[0]; }

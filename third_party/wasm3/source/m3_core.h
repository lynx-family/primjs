...
#define d_m3Log(CATEGORY, FMT, ...) \
  printf(" %8s  |  " FMT, #CATEGORY, ##__VA_ARGS__);
...
typedef struct M3ModuleList {
  IM3Module module;
  struct M3ModuleList *next;
} M3ModuleList;
...
#define d_m3CodePageFreeLinesThreshold \
  (4 + 2)  // max is: select _sss & CallIndirect + 2 for bridge
...
static const char *const c_waTypes[] = {"nil", "i32", "i64",
                                        "f32", "f64", "unknown"};
...
#define ErrorRuntime(RESULT, RUNTIME, FORMAT, ...)                  \
  _m3Error(RESULT, RUNTIME, NULL, NULL, __FILE__, __LINE__, FORMAT, \
           ##__VA_ARGS__)
#define ErrorModule(RESULT, MOD, FORMAT, ...)                           \
  _m3Error(RESULT, MOD->runtime, MOD, NULL, __FILE__, __LINE__, FORMAT, \
           ##__VA_ARGS__)
#define ErrorCompile(RESULT, COMP, FORMAT, ...)                           \
  _m3Error(RESULT, COMP->runtime, COMP->module, NULL, __FILE__, __LINE__, \
           FORMAT, ##__VA_ARGS__)
...
// Tracing format: timestamp;heap:OpCode;name;size(bytes);new items;new ptr;old
// items;old ptr
...
static inline void *m3_ReallocArray_Impl(ccstr_t name, void *i_ptr_old,
                                         size_t i_num_new, size_t i_num_old,
                                         size_t i_size) {
  void *result =
...
#define m3_AllocStruct(STRUCT) \
  (STRUCT *)m3_AllocStruct_Impl(#STRUCT, sizeof(STRUCT))
#define m3_AllocArray(STRUCT, NUM) \
  (STRUCT *)m3_AllocArray_Impl(#STRUCT, NUM, sizeof(STRUCT))
#define m3_ReallocArray(STRUCT, PTR, NEW, OLD)                         \
  (STRUCT *)m3_ReallocArray_Impl(#STRUCT, (void *)(PTR), (NEW), (OLD), \
                                 sizeof(STRUCT))
#define m3_Free(P)                                                           \
  do {                                                                       \
    void *p = (void *)(P);                                                   \
...
#define m3_AllocArray(STRUCT, NUM) \
  (STRUCT *)m3_Malloc_Impl(sizeof(STRUCT) * (NUM))
#define m3_ReallocArray(STRUCT, PTR, NEW, OLD)                     \
  (STRUCT *)m3_Realloc_Impl((void *)(PTR), sizeof(STRUCT) * (NEW), \
                            sizeof(STRUCT) * (OLD))
#define m3_Free(P)             \
  do {                         \
    m3_Free_Impl((void *)(P)); \
...
M3Result ReadLebUnsigned(u64 *o_value, u32 i_maxNumBits, bytes_t *io_bytes,
                         cbytes_t i_end);
M3Result ReadLebSigned(i64 *o_value, u32 i_maxNumBits, bytes_t *io_bytes,
                       cbytes_t i_end);
...
void ReportError(IM3Runtime io_runtime, IM3Module i_module,
                 IM3Function i_function, ccstr_t i_errorMessage, ccstr_t i_file,
                 u32 i_lineNum);

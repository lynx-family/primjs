...
load('test/mjsunit.js');
load('test/wasm-module-builder.js');

// Basic tests.

const outOfUint32RangeValue = 1e12;
// const kV8MaxWasmTableSize = 4294967295;
const kV8TableInitialUpperBound = 10000000;
const kV8MaxWasmTableSize = 100000;
...
(function TestConstructor() {
  ...
  assertThrows(() => new WebAssembly.Table({ element: 'anyfunc', initial: -1 }), TypeError);
  assertThrows(
    () => new WebAssembly.Table({ element: 'anyfunc', initial: outOfUint32RangeValue }),
    TypeError,
  );
  assertThrows(() => new WebAssembly.Table({ element: 'anyfunc', initial: 10, maximum: -1 }), TypeError);
  assertThrows(
    () => new WebAssembly.Table({ element: 'anyfunc', initial: 10, maximum: outOfUint32RangeValue }),
    TypeError,
  );
  ...
  // table = new WebAssembly.Table({ element: 'anyfunc', initial: 0, maxilmum: kV8MaxWasmTabeSize + 1 });
  // assertTableIsValid(table, 0);

  assertThrows(() => {
    table = new WebAssembly.Table({ element: 'anyfunc', initial: kV8TableInitialUpperBound + 1 });
  }, RangeError);
})();
...
(function TestMaximumIsReadOnce() {
  ...
  Object.defineProperty(desc, 'maximum', {
    get: function () {
      if (a) {
        a = false;
        return 16;
      } else {
        // Change the return value on the second call so it throws.        return -1;
      }
    },
  });
...
(function TestMaximumDoesHasProperty() {
  console.log('testing maximumhasproperyt...');
...
(function TestGet() {
  ...
  for (let key of [-1, NaN, {}, () => {}]) {
    assertThrows(() => table.get(key), Error);
  }
...
(function TestSet() {
  ...
  for (let f of [wasm, host]) {
    ...
    assertThrows(() => WebAssembly.Table.prototype.set.call([], 0, f), TypeError);
  }
})();
...
// NOTE: GrowWithInit is not supported yet.

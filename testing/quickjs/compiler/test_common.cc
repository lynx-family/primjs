...
TEST_F(CommonQjsTest, binary_op) {
  ...
  ASSERT_TRUE(LEPUS_VALUE_IS_FLOAT64(d));
  ASSERT_TRUE(LEPUS_VALUE_GET_FLOAT64(d) == -2);
  ASSERT_TRUE(LEPUS_VALUE_IS_FLOAT64(e));
  ASSERT_TRUE(LEPUS_VALUE_GET_FLOAT64(e) == -1);
  ASSERT_TRUE(LEPUS_VALUE_IS_FLOAT64(g));
  ASSERT_TRUE(LEPUS_VALUE_GET_FLOAT64(g) == 2);
  ASSERT_TRUE(LEPUS_VALUE_IS_FLOAT64(h));
  ASSERT_TRUE(LEPUS_VALUE_GET_FLOAT64(h) == 1);
  ASSERT_TRUE(LEPUS_VALUE_IS_FLOAT64(hash1));
  ASSERT_TRUE(LEPUS_VALUE_IS_FLOAT64(hash2));
  ASSERT_TRUE(LEPUS_VALUE_GET_FLOAT64(i));
  ASSERT_TRUE(LEPUS_VALUE_GET_FLOAT64(i) == 2147483647);
  if (!ctx_->rt->gc_enable) LEPUS_FreeValue(ctx_, global);
  return;
}

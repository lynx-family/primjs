...
static napi_value wrap_first_arg(napi_env env, napi_callback_info info,
                                 napi_finalize finalizer, void* data) {
  size_t argc = 1;
  napi_value to_wrap;
...
static napi_value testAdjustExternalMemory(napi_env env,
  ...
  NAPI_CALL(env, napi_adjust_external_memory(env, 100, &adjustedValue));
...
static napi_value add_finalizer_only(napi_env env, napi_callback_info info) {
  ...
  NAPI_CALL(env, napi_add_finalizer(env, argv[0], js_cb_ref,
                                    finalizer_only_callback, NULL, NULL));
  return NULL;
}
...
napi_value Init(napi_env env, napi_value exports) {
  napi_property_descriptor descriptors[] = {
      ...
      DECLARE_NAPI_PROPERTY("testAdjustExternalMemory",

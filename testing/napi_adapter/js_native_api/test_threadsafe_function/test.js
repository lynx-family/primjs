...
// if (process.argv[2] === 'child') {
  ...
  //     binding.Unref();
  //   }
  // }, false /* abort */, true /* launchSecondary */, 100);
...
new Promise(function testWithoutJSMarshaller(resolve) {
  ...
  // Delay for 1000ms to give the child thread a chance to add tasks to the queue. Otherwise, the test suite will exit directly if the queue is empty.
  const start = Date.now();
...
.then((result) => {
  assert.strictEqual(result.indexOf(0), -1)
})

...
static std::thread std_threads[2];
...
static void secondary_thread(void* data) {
  napi_threadsafe_function ts_fn_temp =
      reinterpret_cast<napi_threadsafe_function>(data);

  if (napi_release_threadsafe_function(ts_fn_temp, napi_tsfn_release) !=
      napi_ok) {
...
static void data_source_thread(void* data) {
  ...
  if (napi_get_threadsafe_function_context(ts_fn_temp, &hint) != napi_ok) {
  ...
  if (ts_fn_info->start_secondary) {
    if (napi_acquire_threadsafe_function(ts_fn_temp) != napi_ok) {
      napi_fatal_error("data_source_thread", NAPI_AUTO_LENGTH,
    ...
    try {
      std_threads[1] = std::thread(secondary_thread, ts_fn_temp);
    } catch (...) {
      napi_fatal_error("data_source_thread", NAPI_AUTO_LENGTH,
  ...
  for (index = ARRAY_LENGTH - 1; index > -1 && !queue_was_closing; index--) {
    status = napi_call_threadsafe_function(ts_fn_temp, &ints[index],
  ...
  if (!queue_was_closing && napi_release_threadsafe_function(
                                ts_fn_temp, napi_tsfn_release) != napi_ok) {
    napi_fatal_error("data_source_thread", NAPI_AUTO_LENGTH,
                     "napi_release_threadsafe_function failed",
                     NAPI_AUTO_LENGTH);
  }
...
static napi_value StopThread(napi_env env, napi_callback_info info) {
  ...
  NAPI_CALL(env, napi_release_threadsafe_function(
...
static void join_the_threads(napi_env env, void* data, void* hint) {
  std::thread* the_threads = static_cast<std::thread*>(data);
  ts_fn_hint* the_hint = reinterpret_cast<ts_fn_hint*>(hint);
  napi_value js_cb, undefined;

  if (the_threads[0].joinable()) {
    the_threads[0].join();
  }
  if (the_hint->start_secondary && the_threads[1].joinable()) {
    the_threads[1].join();
  }
  ...
  if (the_hint->js_finalize_cb) {
    ...
    the_hint->js_finalize_cb = NULL;
  }
}

static int start_thread_call_count = 0;
static napi_value StartThreadInternal(napi_env env, napi_callback_info info,
  ...
  NAPI_CALL(env, napi_create_threadsafe_function(
                     env, argv[0], NULL, async_name, ts_info.max_queue_size, 2,
                     std_threads, join_the_threads, &ts_info, cb, &ts_fn));
  bool abort;
  NAPI_CALL(env, napi_get_value_bool(env, argv[1], &abort));
  ...
  try {
    std_threads[0] = std::thread(data_source_thread, ts_fn);
  } catch (...) {
    napi_fatal_error("StartThreadInternal", NAPI_AUTO_LENGTH,
                     "Thread creation failed", NAPI_AUTO_LENGTH);
  }
...
static void ClearStaticVar(void* data) {
  ASSERT_EQ(start_thread_call_count, 13);
  start_thread_call_count = 0;
  ts_fn = NULL;
  if (std_threads[0].joinable()) {
    std_threads[0].join();
  }
  if (std_threads[1].joinable()) {
    std_threads[1].join();
  }
  ts_info.block_on_full = static_cast<napi_threadsafe_function_call_mode>(0);
  ts_info.abort = static_cast<napi_threadsafe_function_release_mode>(0);
  ts_info.start_secondary = false;
  ts_info.js_finalize_cb = nullptr;
  ts_info.max_queue_size = 0;
...
static napi_value Init(napi_env env, napi_value exports) {
  ...
  NAPI_CALL(env, napi_define_properties(
                     ...
                     properties));
  NAPI_CALL(env, napi_add_env_cleanup_hook(env, ClearStaticVar, NULL));

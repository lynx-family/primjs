...
static napi_value throwExistingError(napi_env env, napi_callback_info info) {
  napi_value message;
  napi_value error;
  NAPI_CALL(env, napi_create_string_utf8(env, "existing error",
...
static napi_value throwRangeErrorCode(napi_env env, napi_callback_info info) {
  NAPI_CALL(env, napi_throw_range_error(env, "ERR_TEST_CODE",
                                        "RangeError [range error]"));
  return NULL;
}

static napi_value throwTypeErrorCode(napi_env env, napi_callback_info info) {
  NAPI_CALL(env, napi_throw_type_error(env, "ERR_TEST_CODE",
...
static napi_value createRangeError(napi_env env, napi_callback_info info) {
  napi_value result;
  napi_value message;
  NAPI_CALL(env, napi_create_string_utf8(env, "range error", NAPI_AUTO_LENGTH,
                                         &message));
...
static napi_value createTypeError(napi_env env, napi_callback_info info) {
  napi_value result;
  napi_value message;
  NAPI_CALL(env, napi_create_string_utf8(env, "type error", NAPI_AUTO_LENGTH,
                                         &message));
...
static napi_value createErrorCode(napi_env env, napi_callback_info info) {
  ...
  napi_value message;
  napi_value code;
  NAPI_CALL(env, napi_create_string_utf8(env, "Error [error]", NAPI_AUTO_LENGTH,
                                         &message));
  NAPI_CALL(env, napi_create_string_utf8(env, "ERR_TEST_CODE", NAPI_AUTO_LENGTH,
                                         &code));
...
static napi_value createRangeErrorCode(napi_env env, napi_callback_info info) {
  ...
  napi_value message;
  napi_value code;
  NAPI_CALL(env, napi_create_string_utf8(env, "RangeError [range error]",
                                         NAPI_AUTO_LENGTH, &message));
  NAPI_CALL(env, napi_create_string_utf8(env, "ERR_TEST_CODE", NAPI_AUTO_LENGTH,
                                         &code));
...
static napi_value createTypeErrorCode(napi_env env, napi_callback_info info) {
  ...
  napi_value message;
  napi_value code;
  NAPI_CALL(env, napi_create_string_utf8(env, "TypeError [type error]",
                                         NAPI_AUTO_LENGTH, &message));
  NAPI_CALL(env, napi_create_string_utf8(env, "ERR_TEST_CODE", NAPI_AUTO_LENGTH,
                                         &code));

...
const test_error = require(`./build/export`);
...
assert.throws(
  ...
  (err) => {
    assert.strictEqual(err.code, 'ERR_TEST_CODE');
...
assert.throws(
  ...
  (err) => {
    assert.strictEqual(err.code, 'ERR_TEST_CODE');
    assert.strictEqual(err.message, 'RangeError [range error]');
    return true;
  });
...
assert.throws(
  ...
  (err) => {
    assert.strictEqual(err.code, 'ERR_TEST_CODE');
    assert.strictEqual(err.message, 'TypeError [type error]');
    return true;
  });

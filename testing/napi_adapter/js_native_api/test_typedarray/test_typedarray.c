...
static napi_value External(napi_env env, napi_callback_info info) {
  ...
  napi_value output_array;
  NAPI_CALL(env, napi_create_typedarray(env, napi_int8_array, nElem,
...
static napi_value NullArrayBuffer(napi_env env, napi_callback_info info) {
  ...
  NAPI_CALL(env, napi_create_external_arraybuffer(env, data, 0, NULL, NULL,
                                                  &arraybuffer));
  return arraybuffer;
}
...
//
//  napi_value arraybuffer;
//  NAPI_CALL(env, napi_get_typedarray_info(env, args[0], NULL, NULL, NULL,
//                                          &arraybuffer, NULL));
...
//
//  bool is_detached;
//  NAPI_CALL(env, napi_is_detached_arraybuffer(env, array_buffer,
//  &is_detached));
//
//  napi_value result;

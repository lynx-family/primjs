...
#define GEN_NULL_CHECK_BINDING(binding_name, output_type, api)               \
  static napi_value binding_name(napi_env env, napi_callback_info info) {    \
    ...
    add_returned_status(env, "envIsNull", return_value, "Invalid argument",  \
                        napi_invalid_arg, api(NULL, return_value, &result)); \
    api(env, NULL, &result);                                                 \
...
#define GEN_NULL_CHECK_STRING_BINDING(binding_name, arg_type, api)             \
  static napi_value binding_name(napi_env env, napi_callback_info info) {      \
    ...
    napi_value string;                                                         \
    NAPI_CALL(env, napi_create_string_utf8(env, "Something", NAPI_AUTO_LENGTH, \
                                           &string));                          \
...
GEN_NULL_CHECK_STRING_BINDING(GetValueStringUtf8, char,
                              napi_get_value_string_utf8)
GEN_NULL_CHECK_STRING_BINDING(GetValueStringLatin1, char,
                              napi_get_value_string_latin1)
GEN_NULL_CHECK_STRING_BINDING(GetValueStringUtf16, char16_t,
                              napi_get_value_string_utf16)

void init_test_null_conversions(napi_env env, napi_value exports) {

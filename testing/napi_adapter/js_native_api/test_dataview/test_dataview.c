...
static napi_value CreateDataView(napi_env env, napi_callback_info info) {
  ...
  NAPI_CALL(env,
            napi_get_value_uint32(env, args[1], (uint32_t*)(&byte_offset)));
  ...
  NAPI_CALL(env, napi_create_dataview(env, length, arraybuffer, byte_offset,
                                      &output_dataview));

  return output_dataview;
}

static napi_value CreateDataViewFromJSDataView(napi_env env,
  ...
  size_t length = 0;
  napi_value buffer;
  NAPI_CALL(env, napi_get_dataview_info(env, input_dataview, &length, NULL,
                                        &buffer, &byte_offset));

  napi_value output_dataview;
  NAPI_CALL(env, napi_create_dataview(env, length, buffer, byte_offset,
                                      &output_dataview));

...
static napi_value HasNamedProperty(napi_env env, napi_callback_info info) {
  ...
  char buffer[128];
  size_t copied;
  NAPI_CALL(env, napi_get_value_string_utf8(env, args[1], buffer,
                                            sizeof(buffer), &copied));
...
napi_value Init(napi_env env, napi_value exports) {
  ...
  NAPI_CALL(env, napi_create_string_utf8(env, "NameKeyValue", NAPI_AUTO_LENGTH,
                                         &name_value));

  napi_value symbol_description;
  napi_value name_symbol;
  NAPI_CALL(env, napi_create_string_utf8(env, "NameKeySymbol", NAPI_AUTO_LENGTH,
                                         &symbol_description));
  NAPI_CALL(env, napi_create_symbol(env, symbol_description, &name_symbol));

  napi_property_descriptor properties[] = {
      {"echo", 0, Echo, 0, 0, 0, napi_enumerable, 0},
      {"readwriteValue", 0, 0, 0, 0, number, napi_enumerable | napi_writable,
       0},

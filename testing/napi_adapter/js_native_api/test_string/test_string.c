...
static napi_value TestLatin1(napi_env env, napi_callback_info info) {
  ...
  NAPI_CALL(env, napi_get_value_string_latin1(env, args[0], buffer, buffer_size,
                                              &copied));
...
static napi_value TestUtf16(napi_env env, napi_callback_info info) {
  ...
  NAPI_CALL(env, napi_get_value_string_utf16(env, args[0], buffer, buffer_size,
                                             &copied));
...
static napi_value TestUtf8Insufficient(napi_env env, napi_callback_info info) {
  ...
  NAPI_CALL(env, napi_get_value_int32(env, args[1], &copy_length));

  NAPI_ASSERT(env, copy_length >= 0 && copy_length <= 3,
              "Wrong number of copied length");

  char buffer[4] = {0, 0, 0, 0};
  size_t buffer_size = copy_length + 1;
  size_t copied = 0;
...
static napi_value TestLargeLatin1(napi_env env, napi_callback_info info) {
  napi_value output;
  if (SIZE_MAX > INT_MAX) {
    NAPI_CALL(env, napi_create_string_latin1(env, "", ((size_t)INT_MAX) + 1,
                                             &output));
  } else {
    // just throw the expected error as there is nothing to test
    // in this case since we can't overflow
...
static napi_value TestLargeUtf16(napi_env env, napi_callback_info info) {
  napi_value output;
  if (SIZE_MAX > INT_MAX) {
    NAPI_CALL(env, napi_create_string_utf16(env, ((const char16_t*)""),
...
napi_value Init(napi_env env, napi_value exports) {
  ...
  NAPI_CALL(env, napi_define_properties(
                     env, exports, sizeof(properties) / sizeof(*properties),
                     properties));

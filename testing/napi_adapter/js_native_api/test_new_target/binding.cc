...
static napi_value BaseClass(napi_env env, napi_callback_info info) {
  ...
  NAPI_CALL(
      env,
      napi_define_class(
          env, "BaseClass", NAPI_AUTO_LENGTH,
          [](napi_env env, napi_callback_info info) -> napi_value {
            ...
            bool result;
            NAPI_CALL(env,
                      napi_strict_equals(env, newTargetArg, thisArg, &result));
            NAPI_ASSERT(env, !result, "this !== new.target");

            // new.target !== undefined because we should be called as a new
            // expression
            NAPI_ASSERT(env, newTargetArg != NULL, "newTargetArg != NULL");
            NAPI_CALL(
                env, napi_strict_equals(env, newTargetArg, undefined, &result));
            NAPI_ASSERT(env, !result, "new.target !== undefined");
            return thisArg;
          },
          NULL, 0, NULL, &base_class));
  ...
  NAPI_CALL(
      env, napi_define_class(
               ...
               [](napi_env env, napi_callback_info info) -> napi_value {
                 ...
                 napi_value argv;
                 napi_value thisArg;
                 NAPI_CALL(env, napi_get_cb_info(env, info, &argc, &argv,
                                                 &thisArg, NULL));
                 napi_value undefined;
                 NAPI_CALL(env, napi_get_undefined(env, &undefined));

                 // new.target !== undefined because we should be called as a
                 // new expression
                 NAPI_ASSERT(env, newTargetArg != NULL, "newTargetArg != NULL");
                 NAPI_CALL(env, napi_strict_equals(env, newTargetArg, undefined,
                                                   &result));
                 NAPI_ASSERT(env, !result, "new.target !== undefined");

                 // arguments[0] should be Constructor itself (test harness
                 // passed it)
                 NAPI_CALL(
                     env, napi_strict_equals(env, newTargetArg, argv, &result));

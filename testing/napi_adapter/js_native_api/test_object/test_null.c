...
static napi_value TestBoolValuedPropApi(napi_env env,
                                        napi_status (*api)(napi_env, napi_value,
                                                           napi_value, bool*)) {
  napi_value return_value, object, key;
  bool result;
  ...
  add_returned_status(env, "envIsNull", return_value, "Invalid argument",
                      napi_invalid_arg, api(NULL, object, key, &result));
...
static napi_value TestBoolValuedElementApi(
  ...
  add_returned_status(env, "envIsNull", return_value, "Invalid argument",
                      napi_invalid_arg, api(NULL, object, 0, &result));
...
static napi_value DefineProperties(napi_env env, napi_callback_info info) {
  ...
  napi_property_descriptor desc = {"prop", NULL, DefineProperties, NULL,
...
static napi_value GetAllPropertyNames(napi_env env, napi_callback_info info) {
  ...
  add_returned_status(
      env, "envIsNull", return_value, "Invalid argument", napi_invalid_arg,
      napi_get_all_property_names(NULL, return_value, napi_key_own_only,
                                  napi_key_writable, napi_key_keep_numbers,
                                  &props));

  napi_get_all_property_names(env, NULL, napi_key_own_only, napi_key_writable,
                              napi_key_keep_numbers, &props);
  add_last_status(env, "objectIsNull", return_value);

  napi_get_all_property_names(env, return_value, napi_key_own_only,
                              napi_key_writable, napi_key_keep_numbers, NULL);
  add_last_status(env, "valueIsNull", return_value);
...
void init_test_null_object(napi_env env, napi_value exports) {
  ...
  NAPI_CALL_RETURN_VOID(
      env,
      napi_define_properties(env, test_null,

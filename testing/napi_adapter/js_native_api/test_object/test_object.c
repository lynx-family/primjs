...
static napi_value GetSymbolNames(napi_env env, napi_callback_info info) {
  ...
  napi_value output;
  NAPI_CALL(
      env, napi_get_all_property_names(
...
static napi_value TestSetProperty(napi_env env, napi_callback_info info) {
  ...
  add_returned_status(env, "envIsNull", object, "Invalid argument",
...
//   NAPI_CALL(env, napi_get_cb_info(env, info, &argc, &which_type, NULL,
//   NULL)); NAPI_CALL(env, napi_get_value_uint32(env, which_type,
//   &type_index)); NAPI_CALL(env, napi_create_object(env, &instance));
//   NAPI_CALL(env, napi_type_tag_object(env, instance,
...
napi_value Init(napi_env env, napi_value exports) {
  ...
  init_test_null_object(env, exports);

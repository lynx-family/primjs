...
static napi_value TestDefineClass(napi_env env, napi_callback_info info) {
  ...
  status = napi_define_class(NULL, "TrackedFunction", NAPI_AUTO_LENGTH,
                             TestDefineClass, NULL, 1, &property_descriptor,
                             &result);

  add_returned_status(env, "envIsNull", return_value, "Invalid argument",
                      napi_invalid_arg, status);

  napi_define_class(env, NULL, NAPI_AUTO_LENGTH, TestDefineClass, NULL, 1,
                    &property_descriptor, &result);

  add_last_status(env, "nameIsNull", return_value);

  napi_define_class(env, "TrackedFunction", NAPI_AUTO_LENGTH, NULL, NULL, 1,
                    &property_descriptor, &result);

  add_last_status(env, "cbIsNull", return_value);

  napi_define_class(env, "TrackedFunction", NAPI_AUTO_LENGTH, TestDefineClass,
                    NULL, 1, &property_descriptor, &result);
  ...
  napi_define_class(env, "TrackedFunction", NAPI_AUTO_LENGTH, TestDefineClass,
                    NULL, 1, &property_descriptor, NULL);
...
napi_value Init(napi_env env, napi_value exports) {
  ...
  NAPI_CALL(env, napi_define_class(env, "MyObject_Extra", NAPI_AUTO_LENGTH,
                                   NewExtra, NULL, 0, NULL, &cons));
  ...
  NAPI_CALL(env, napi_define_class(env, "MyObject", NAPI_AUTO_LENGTH, New, NULL,
